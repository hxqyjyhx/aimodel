"""
Block 1J43c-r2 -- BabyAI Rediscovered Target Approach Repair.

This is a narrow BabyAI probe repair focused on the transition from
target-not-visible exploration to rediscovered-target local approach. It keeps
the legal observation / action / outcome conversion, mission parser,
visible-target bridge, and exploration fallback from 1J43c-r as much as
possible, but adds explicit rediscovery-approach tracking and local approach
repair.
...
SCRIPT_NAME = "_block1j43c_r2_babyai_rediscovered_target_approach_repair.py"
RUN_JSON = os.path.join(CURRENT_DIR, "runs", "block1j43c_r2_babyai_rediscovered_target_approach_repair.json")
REPORT_MD = os.path.join(CURRENT_DIR, "protocols", "block1j43c_r2_babyai_rediscovered_target_approach_repair.md")
TABLE_CSV = os.path.join(CURRENT_DIR, "protocols", "block1j43c_r2_babyai_rediscovered_target_approach_repair_table.csv")
CHECKPOINT_MD = os.path.join(CURRENT_DIR, "checkpoint_1j43c_r2_babyai_rediscovered_target_approach_repair.md")
...
BASELINE_1J43C_R_RETURN = 0.2189
BASELINE_REDISCOVERED_TO_ADJACENT_RATE = 0.1730
BASELINE_REDISCOVERED_TO_INTERACTION_RATE = 0.1503
CANDIDATE_ENVS = [
    "BabyAI-GoToObj-v0",
...
def local_target_distance(target_cell):
    if target_cell is None:
        return None
    return abs(int(target_cell["x"]) - FRONT_X) + abs(int(target_cell["y"]) - FRONT_Y)


def side_open_score(image, side):
    if side == "left":
        xs = range(0, FRONT_X)
    else:
        xs = range(FRONT_X + 1, image.shape[0])
    score = 0
    for x in xs:
        for y in range(0, FRONT_Y + 1):
...
def reset_after_rediscovery(policy_state, current_signature):
    ...
    policy_state["rediscovered_target_lock"] = True
    policy_state["rediscovered_reacquire_budget"] = 2
    policy_state["rediscovered_last_seen_x"] = None
    policy_state["rediscovered_last_seen_y"] = None
    policy_state["rediscovered_last_distance"] = None
    policy_state["rediscovered_last_recovery_turn"] = None
...
def bridge_action_from_target_repaired(observation, parsed_mission, target_cell, policy_state):
    ...
    target_distance = local_target_distance(target_cell)

    if target_cell is None:
        policy_state["rediscovered_target_lock"] = False
        ...
        return action_id, {
            ...
            "front_blocked": False,
            "recovery_turn": False,
            "approach_progress_hint": False,
        }

    policy_state["bridge_priority_steps"] = max(policy_state.get("bridge_priority_steps", 0), 1)
    policy_state["rediscovered_target_lock"] = True
    policy_state["rediscovered_reacquire_budget"] = 2
    policy_state["rediscovered_last_seen_x"] = int(target_cell["x"])
    policy_state["rediscovered_last_seen_y"] = int(target_cell["y"])
    policy_state["rediscovered_last_distance"] = target_distance
    ...
    left_score = side_open_score(image, "left")
    right_score = side_open_score(image, "right")
    prefer_left = left_score >= right_score
    policy_state["rediscovered_last_recovery_turn"] = "turn_left" if prefer_left else "turn_right"
    return bridge_turn_action(policy_state, prefer_left=prefer_left), {
        "target_visible": True,
        "target_adjacent": target_adjacent,
        "target_front": False,
        "used_bridge": True,
...
def choose_rediscovered_target_completion_action(observation, parsed_mission, experience_records, policy_state, rng):
    ...
    if policy_state.get("rediscovered_target_lock") and policy_state.get("rediscovered_reacquire_budget", 0) > 0:
        policy_state["rediscovered_reacquire_budget"] -= 1
        last_x = policy_state.get("rediscovered_last_seen_x")
        last_recovery_turn = policy_state.get("rediscovered_last_recovery_turn")
        if last_x is not None and last_x < FRONT_X:
            action_id = ACTION_NAME_TO_ID["turn_left"]
        elif last_x is not None and last_x > FRONT_X:
            action_id = ACTION_NAME_TO_ID["turn_right"]
        elif last_recovery_turn in ACTION_NAME_TO_ID:
            action_id = ACTION_NAME_TO_ID[last_recovery_turn]
        else:
            image = observation["image"]
            front_cell = get_cell(image, FRONT_X, FRONT_Y)
            action_id = ACTION_NAME_TO_ID["move_forward"] if is_passable(front_cell) else bridge_turn_action(policy_state)
        return action_id, {
            "target_visible": False,
            "target_adjacent": False,
            "target_front": False,
            ...
            "approach_action": True,
            "front_blocked": False,
            "recovery_turn": ACTION_ID_TO_NAME[int(action_id)] in {"turn_left", "turn_right"},
            "approach_progress_hint": False,
            "reacquire_action": True,
        }
...
def collect_trajectory(env, seed, policy_name, experience_records=None):
    ...
    policy_state = {
        ...
        "rediscovered_target_lock": False,
        "rediscovered_reacquire_budget": 0,
        "rediscovered_last_seen_x": None,
        "rediscovered_last_seen_y": None,
        "rediscovered_last_distance": None,
        "rediscovered_last_recovery_turn": None,
    }
    ...
    rediscovered_target_visible_steps = 0
    rediscovered_target_lost_count = 0
    rediscovered_target_lost_before_adjacent = False
    rediscovered_front_blocked_count = 0
    rediscovered_recovery_turn_count = 0
    rediscovered_approach_action_count = 0
    rediscovered_approach_progress_count = 0
    rediscovered_progress_denominator = 0
    failure_flags = {
        "not_approached_after_rediscovery": False,
        "target_lost_before_adjacent": False,
        "front_blocked_after_rediscovery": False,
    ...
    while not done:
        ...
        current_target_distance = local_target_distance(current_target_cell)
        previous_rediscovery_distance = policy_state.get("rediscovered_last_distance")
        ...
        elif policy_state["rediscovered_target_seen"] and (not was_visible_previous) and current_target_visible:
            current_rediscovery_window = True
            policy_state["rediscovered_target_lock"] = True
            policy_state["rediscovered_reacquire_budget"] = 2
        ...
        if policy_state["rediscovered_target_seen"] and was_visible_previous and not current_target_visible:
            rediscovered_target_lost_count += 1
            if not saw_adjacent_after_rediscovery:
                rediscovered_target_lost_before_adjacent = True
                failure_flags["target_lost_before_adjacent"] = True
            policy_state["rediscovered_target_lock"] = True
            policy_state["rediscovered_reacquire_budget"] = max(policy_state.get("rediscovered_reacquire_budget", 0), 2)
        ...
        action_info.setdefault("approach_action", False)
        action_info.setdefault("front_blocked", False)
        action_info.setdefault("recovery_turn", False)
        action_info.setdefault("approach_progress_hint", False)

        if current_rediscovery_window and current_target_visible:
            rediscovered_target_visible_steps += 1
            ...
            if action_info["front_blocked"]:
                rediscovered_front_blocked_count += 1
                failure_flags["front_blocked_after_rediscovery"] = True
            if action_info["recovery_turn"]:
                rediscovered_recovery_turn_count += 1
            if action_info["approach_action"]:
                rediscovered_approach_action_count += 1
            if (
                current_target_distance is not None
                and previous_rediscovery_distance is not None
                and current_target_distance < previous_rediscovery_distance
            ):
                rediscovered_approach_progress_count += 1
            if action_info["approach_action"]:
                rediscovered_progress_denominator += 1
    ...
    return {
        ...
        "rediscovered_target_visible_steps": rediscovered_target_visible_steps,
        "rediscovered_target_lost_count": rediscovered_target_lost_count,
        "rediscovered_target_lost_before_adjacent": rediscovered_target_lost_before_adjacent,
        "rediscovered_front_blocked_count": rediscovered_front_blocked_count,
        "rediscovered_recovery_turn_count": rediscovered_recovery_turn_count,
        "rediscovered_approach_action_count": rediscovered_approach_action_count,
        "rediscovered_approach_progress_rate": (
            rediscovered_approach_progress_count / max(1, rediscovered_progress_denominator)
        ),
...
def summarize_policy(trajectories):
    ...
    rediscovered_visible_steps = sum(traj["rediscovered_target_visible_steps"] for traj in trajectories)
    rediscovered_target_lost_count = sum(traj["rediscovered_target_lost_count"] for traj in trajectories)
    rediscovered_front_blocked_count = sum(traj["rediscovered_front_blocked_count"] for traj in trajectories)
    rediscovered_recovery_turn_count = sum(traj["rediscovered_recovery_turn_count"] for traj in trajectories)
    rediscovered_approach_action_count = sum(traj["rediscovered_approach_action_count"] for traj in trajectories)
    rediscovered_approach_progress_rate = mean(
        [traj["rediscovered_approach_progress_rate"] for traj in rediscovered_episodes]
    )
    ...
    return {
        ...
        "rediscovered_target_visible_steps": rediscovered_visible_steps,
        "rediscovered_target_lost_count": rediscovered_target_lost_count,
        "rediscovered_target_lost_before_adjacent_rate": mean(
            [1.0 if traj["rediscovered_target_lost_before_adjacent"] else 0.0 for traj in rediscovered_episodes]
        ),
        "rediscovered_front_blocked_count": rediscovered_front_blocked_count,
        "rediscovered_recovery_turn_count": rediscovered_recovery_turn_count,
        "rediscovered_approach_action_count": rediscovered_approach_action_count,
        "rediscovered_approach_progress_rate": rediscovered_approach_progress_rate,
        ...
        "target_lost_before_adjacent": failure_breakdown["target_lost_before_adjacent"],
        "front_blocked_after_rediscovery": failure_breakdown["front_blocked_after_rediscovery"],
...
def run_env_seed(env_id, seed):
    ...
    return {
        ...
        "rediscovered_target_approach_probe_return": policy_summary["rediscovered_target_completion_probe"]["return"],
        ...
        "delta_vs_1j43c_r": policy_summary["rediscovered_target_completion_probe"]["return"] - BASELINE_1J43C_R_RETURN,
        ...
        "rediscovered_target_visible_steps": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_target_visible_steps"
        ],
        "rediscovered_target_lost_count": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_target_lost_count"
        ],
        "rediscovered_target_lost_before_adjacent_rate": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_target_lost_before_adjacent_rate"
        ],
        "rediscovered_front_blocked_count": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_front_blocked_count"
        ],
        "rediscovered_recovery_turn_count": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_recovery_turn_count"
        ],
        "rediscovered_approach_action_count": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_approach_action_count"
        ],
        "rediscovered_approach_progress_rate": policy_summary["rediscovered_target_completion_probe"][
            "rediscovered_approach_progress_rate"
        ],
        ...
        "target_lost_before_adjacent": policy_summary["rediscovered_target_completion_probe"][
            "target_lost_before_adjacent"
        ],
        "front_blocked_after_rediscovery": policy_summary["rediscovered_target_completion_probe"][
            "front_blocked_after_rediscovery"
        ],
...
probe_1j43c_r_return = BASELINE_1J43C_R_RETURN
rediscovered_target_approach_return = agg("rediscovered_target_approach_probe_return")
delta_vs_random = agg("delta_vs_random")
delta_vs_1j43b = agg("delta_vs_1j43b")
delta_vs_1j43c = agg("delta_vs_1j43c")
delta_vs_1j43c_r = agg("delta_vs_1j43c_r")
...
rediscovered_target_visible_steps = agg("rediscovered_target_visible_steps")
rediscovered_target_lost_count = agg("rediscovered_target_lost_count")
rediscovered_target_lost_before_adjacent_rate = agg("rediscovered_target_lost_before_adjacent_rate")
rediscovered_front_blocked_count = agg("rediscovered_front_blocked_count")
rediscovered_recovery_turn_count = agg("rediscovered_recovery_turn_count")
rediscovered_approach_action_count = agg("rediscovered_approach_action_count")
rediscovered_approach_progress_rate = agg("rediscovered_approach_progress_rate")
...
target_lost_before_adjacent = agg("target_lost_before_adjacent")
front_blocked_after_rediscovery = agg("front_blocked_after_rediscovery")
...
any_nonzero_env = any(
    row["rediscovered_target_approach_probe_return"] > 0.0
    or row["success_proxy_rate"] > 0.0
    or row["target_discovery_rate"] > 0.0
    for row in all_rows
) if all_rows else False
approach_improved = rediscovered_to_adjacent_rate > BASELINE_REDISCOVERED_TO_ADJACENT_RATE
interaction_improved = rediscovered_to_interaction_attempt_rate > BASELINE_REDISCOVERED_TO_INTERACTION_RATE
meets_return_or_hidden_success = (
    rediscovered_target_approach_return >= BASELINE_1J43C_R_RETURN
...
if (
    ...
    and approach_improved
    and interaction_improved
    and meets_return_or_hidden_success
    and handoff_active
    ...
    and handoff_active
    and target_discovery_preserved
    and (approach_improved or interaction_improved)
):
    status = "PARTIAL"
else:
    status = "FAIL"
...
payload = {
    ...
    "summary": {
        ...
        "probe_1j43c_r_return": probe_1j43c_r_return,
        "probe_1j43c_r2_return": rediscovered_target_approach_return,
        "delta_vs_random": delta_vs_random,
        "delta_vs_1j43b": delta_vs_1j43b,
        "delta_vs_1j43c": delta_vs_1j43c,
        "delta_vs_1j43c_r": delta_vs_1j43c_r,
        ...
        "rediscovered_target_visible_steps": rediscovered_target_visible_steps,
        "rediscovered_target_lost_count": rediscovered_target_lost_count,
        "rediscovered_target_lost_before_adjacent_rate": rediscovered_target_lost_before_adjacent_rate,
        "rediscovered_front_blocked_count": rediscovered_front_blocked_count,
        "rediscovered_recovery_turn_count": rediscovered_recovery_turn_count,
        "rediscovered_approach_action_count": rediscovered_approach_action_count,
        "rediscovered_approach_progress_rate": rediscovered_approach_progress_rate,
        ...
        "target_lost_before_adjacent": target_lost_before_adjacent,
        "front_blocked_after_rediscovery": front_blocked_after_rediscovery,
        ...
        "main_exploration_failure": "rediscovered handoff stays clean, but local obstacle-sensitive approach still limits adjacency conversion on Pickup/OpenDoor",
...
with open(TABLE_CSV, "w", encoding="utf-8", newline="") as handle:
    writer = csv.DictWriter(
        ...
        fieldnames=[
            ...
            "1j43b_probe_return",
            "1j43c_probe_return",
            "rediscovered_target_approach_probe_return",
            "delta_vs_random",
            "delta_vs_1j43b",
            "delta_vs_1j43c",
            "delta_vs_1j43c_r",
...
report_lines = [
    "# 1J43c-r2 BabyAI Rediscovered Target Approach Repair",
    "",
    "## Checkpoint Summary",
    f"- Status: `{status}`",
    ...
    "",
    "## Commands Run",
    "- `python _block1j43c_r2_babyai_rediscovered_target_approach_repair.py`",
    "",
    "## Adapter Coverage",
    ...
    f"- 1J43c-r return: `{probe_1j43c_r_return:.4f}`",
    f"- 1J43c-r2 return: `{rediscovered_target_approach_return:.4f}`",
    f"- Delta vs random: `{delta_vs_random:+.4f}`",
    f"- Delta vs 1J43c-r: `{delta_vs_1j43c_r:+.4f}`",
    ...
    f"- Rediscovered target visible steps: `{rediscovered_target_visible_steps:.4f}`",
    f"- Rediscovered target lost count: `{rediscovered_target_lost_count:.4f}`",
    f"- Rediscovered target lost before adjacent rate: `{rediscovered_target_lost_before_adjacent_rate:.4f}`",
    f"- Rediscovered front blocked count: `{rediscovered_front_blocked_count:.4f}`",
    f"- Rediscovered recovery turn count: `{rediscovered_recovery_turn_count:.4f}`",
    f"- Rediscovered approach action count: `{rediscovered_approach_action_count:.4f}`",
    f"- Rediscovered approach progress rate: `{rediscovered_approach_progress_rate:.4f}`",
    ...
    f"- Main exploration failure: `rediscovered targets now hand off cleanly, but local obstacle-sensitive approach still limits adjacency conversion`",
    "",
    "## Pickup/OpenDoor Failure Breakdown",
    f"- Not approached after rediscovery: `{not_approached_after_rediscovery:.4f}`",
    f"- Target lost before adjacent: `{target_lost_before_adjacent:.4f}`",
    f"- Front blocked after rediscovery: `{front_blocked_after_rediscovery:.4f}`",
...
checkpoint_lines = [
    ...
    f"- Mission parse success: `{mission_parse_success_rate:.4f}`",
    f"- Hidden leakage: `{str(hidden_leakage).lower()}`",
    "- Next step: BabyAI small pilot if adjacency conversion is now strong enough; otherwise stop local repair and treat the remaining limit as obstacle-sensitive local navigation.",
]
...
print(
    ...
    f"probe_1j43c_r={probe_1j43c_r_return:.4f} probe_1j43c_r2={rediscovered_target_approach_return:.4f} "
    f"delta_random={delta_vs_random:+.4f} delta_1j43c_r={delta_vs_1j43c_r:+.4f} success={success_proxy_rate:.4f} "
...
print(
    ...
    f"stolen_by_fallback={bridge_control_stolen_by_fallback_count:.1f}"
)
print(
    "Rediscovery/Approach: "
    f"visible_steps={rediscovered_target_visible_steps:.1f} lost={rediscovered_target_lost_count:.1f} "
    f"lost_before_adj={rediscovered_target_lost_before_adjacent_rate:.4f} front_blocked={rediscovered_front_blocked_count:.1f} "
    f"recovery_turns={rediscovered_recovery_turn_count:.1f} approach_actions={rediscovered_approach_action_count:.1f} "
    f"progress={rediscovered_approach_progress_rate:.4f} "
    f"adjacent={rediscovered_to_adjacent_rate:.4f} interaction={rediscovered_to_interaction_attempt_rate:.4f} "
    f"success={rediscovered_to_success_rate:.4f}"
)
print(
    "Exploration: "
...
print(
    ...
    f"not_approached={not_approached_after_rediscovery:.1f} target_lost_before_adj={target_lost_before_adjacent:.1f} "
    f"front_blocked={front_blocked_after_rediscovery:.1f} adjacent_not_facing={adjacent_not_facing:.1f} "

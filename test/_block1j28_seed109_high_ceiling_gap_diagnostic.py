...
Patched from 1J24b with seed=103, n_permutation_repeats=100.
Deep-dive into why seed 109 produces weak signed-memory results.
All 1J24b validated fixes preserved.
...
Variants:
  ...
  C_signed_sum_online_permuted_repeat_00..99 -- Online stepwise permuted (100 repeats)
  C_signed_max_abs_online_permuted_repeat_00..99 -- Online stepwise permuted (100 repeats)
...
parser.add_argument("--n-perm-repeats", type=int, default=100, help="Permutation repeats (default: 100)")
...
print("=" * 70)
print("Block 1J28 -- Seed 109 Weak-Seed Diagnostic for Online Signed Memory")
...
print(f"  n_perm_repeats={N_PERM_REPEATS}  output_suffix={args.output_suffix}")
print("=" * 70)
...
# Run a single online stepwise permuted control for baseline comparison (repeat 0)
print("  Running C_signed_sum_online_permuted_r00...")
eps_signed_sum_perm, mem_signed_sum_perm = run_online_stepwise_permuted_variant(
    "C_signed_sum_online_permuted", "signed_sum", 0)
stats_signed_sum_perm = compute_variant_stats(
    eps_signed_sum_perm, all_episodes_a, "C_signed_sum_online_permuted")

print("  Running C_signed_max_abs_online_permuted_r00...")
eps_signed_max_perm, mem_signed_max_perm = run_online_stepwise_permuted_variant(
    "C_signed_max_abs_online_permuted", "signed_max_abs", 0)
...
# --- Permutation distribution (20 repeats per signed variant) ---
print(f"\n  Running permutation distribution (N_PERM_REPEATS={N_PERM_REPEATS})...")
perm_dist_signed_sum = []
perm_dist_signed_max = []

for rep in range(N_PERM_REPEATS):
    print(f"    perm repeat {rep+1}/{N_PERM_REPEATS}...", end=" ")
    eps_ss, mem_ss = run_online_stepwise_permuted_variant(
        "C_signed_sum_online_permuted", "signed_sum", rep)
    eps_sm, mem_sm = run_online_stepwise_permuted_variant(
        "C_signed_max_abs_online_permuted", "signed_max_abs", rep)
    agg_ss = aggregate_episodes(eps_ss)
    agg_sm = aggregate_episodes(eps_sm)
    perm_dist_signed_sum.append(agg_ss["total_prior_violations"])
    perm_dist_signed_max.append(agg_sm["total_prior_violations"])
    print(f"sum_pv={agg_ss['total_prior_violations']} max_pv={agg_sm['total_prior_violations']}")
...
# Compute permutation distribution stats
def perm_dist_stats(pv_list, real_pv):
    arr = sorted(pv_list)
    n = len(arr)
    ...
    return {
        ...
        "all_permuted_pvs": arr,
    }

perm_dist_signed_sum_stats = perm_dist_stats(
    perm_dist_signed_sum, stats_signed_sum["total_prior_violations"])
perm_dist_signed_max_stats = perm_dist_stats(
    perm_dist_signed_max, stats_signed_max["total_prior_violations"])
...
# Leakage fields for online variants (Fix 4)
online_leakage_fields = {
    "offline_oracle_evidence_used": False,
    "true_deceptive_label_used": False,
    "future_outcome_used_for_current_selection": False,
    "same_episode_future_probe_used": False,
}
offline_leakage_fields = {
    "offline_oracle_evidence_used": True,
    "true_deceptive_label_used": False,
    "future_outcome_used_for_current_selection": False,
    "same_episode_future_probe_used": False,
}
...
for name, stats in all_variants.items():
    is_offline = "Offline" in name
    leakage = offline_leakage_fields if is_offline else online_leakage_fields
    part1_rows.append({
        "variant": name,
        ...
        "offline_oracle_evidence_used": leakage["offline_oracle_evidence_used"],
        "true_deceptive_label_used": leakage["true_deceptive_label_used"],
        "future_outcome_used_for_current_selection": leakage["future_outcome_used_for_current_selection"],
        "same_episode_future_probe_used": leakage["same_episode_future_probe_used"],
    })

no_oracle_leakage_confirmed = all(
    not r["offline_oracle_evidence_used"] and
    not r["true_deceptive_label_used"] and
    not r["future_outcome_used_for_current_selection"] and
    not r["same_episode_future_probe_used"]
    for r in part1_rows if "Offline" not in r["variant"]
)
...
# --- Part 4: Permutation distribution ---
part4 = {
    ...
    "signed_sum_permuted_distribution": perm_dist_signed_sum_stats,
    "signed_max_abs_permuted_distribution": perm_dist_signed_max_stats,
    "single_permuted_comparison": [
        {
            "real_variant_name": "B_signed_sum_online",
            "permuted_variant_name": "C_signed_sum_online_permuted_r00",
            "real_total_pv": stats_signed_sum["total_prior_violations"],
            "permuted_r00_total_pv": stats_signed_sum_perm["total_prior_violations"],
            "real_macro_bal": stats_signed_sum["mean_macro_bal"],
            "permuted_r00_macro_bal": stats_signed_sum_perm["mean_macro_bal"],
            "real_beats_permuted_r00": stats_signed_sum["total_prior_violations"] < stats_signed_sum_perm["total_prior_violations"],
        },
        {
            "real_variant_name": "B_signed_max_abs_online",
            "permuted_variant_name": "C_signed_max_abs_online_permuted_r00",
            "real_total_pv": stats_signed_max["total_prior_violations"],
            "permuted_r00_total_pv": stats_signed_max_perm["total_prior_violations"],
            "real_macro_bal": stats_signed_max["mean_macro_bal"],
            "permuted_r00_macro_bal": stats_signed_max_perm["mean_macro_bal"],
            "real_beats_permuted_r00": stats_signed_max["total_prior_violations"] < stats_signed_max_perm["total_prior_violations"],
        },
    ],
}
...
# no_oracle_leakage_confirmed computed from explicit fields (Fix 4)
...
signed_sum_online_beats_permuted_median = signed_sum_pv < perm_dist_signed_sum_stats["median_total_pv"]
signed_sum_online_beats_80pct_of_permuted = (
    perm_dist_signed_sum_stats["num_permuted_worse_than_real"] >= int(N_PERM_REPEATS * 0.8)
)
signed_max_abs_online_beats_A = signed_max_pv < a_total_pv
signed_max_abs_online_beats_permuted_median = signed_max_pv < perm_dist_signed_max_stats["median_total_pv"]
signed_max_abs_online_beats_80pct_of_permuted = (
    perm_dist_signed_max_stats["num_permuted_worse_than_real"] >= int(N_PERM_REPEATS * 0.8)
)
...
if signed_sum_online_beats_A and signed_sum_online_beats_permuted_median:
    implementation_status = "pass"
    failure_reason = "none"
elif signed_sum_online_beats_A and not signed_sum_online_beats_permuted_median:
    implementation_status = "partial"
    failure_reason = "signed_beats_A_but_not_permuted_median"
elif not signed_sum_online_beats_A and signed_sum_online_beats_permuted_median:
    implementation_status = "partial"
    failure_reason = "signed_beats_permuted_median_but_not_A"
else:
    implementation_status = "partial"
    failure_reason = "signed_online_does_not_beat_A"

# =============================================================================
# High-Ceiling Online-Offline Gap Diagnostic (seed 109 specific)
# =============================================================================
# Diagnose why seed109 has high offline improvement room (23) but weak
# online signed_sum improvement (PV=27 vs A=29, reduction of only 2).
high_ceiling_gap_diagnostic = {}

# ---------------------------------------------------------------------------
# Part 1: Confirm 100-repeat permutation distribution
# ---------------------------------------------------------------------------
diag_p1 = {}

# signed_sum
dss = perm_dist_signed_sum_stats
diag_p1["signed_sum"] = {
    "real_total_pv": dss["real_total_pv"],
    "permuted_min_total_pv": dss["min_total_pv"],
    "permuted_max_total_pv": dss["max_total_pv"],
    "permuted_mean_total_pv": dss["mean_total_pv"],
    "permuted_median_total_pv": dss["median_total_pv"],
    "permuted_std_total_pv": dss["std_total_pv"],
    "num_permuted_better_than_real": dss["num_permuted_better_than_real"],
    "num_permuted_equal_to_real": dss.get("num_permuted_equal_to_real", 0),
    "num_permuted_worse_than_real": dss["num_permuted_worse_than_real"],
    "real_percentile_against_permuted": dss["real_percentile_against_permuted"],
    "real_beats_permuted_median": dss["real_total_pv"] < dss["median_total_pv"],
    "real_beats_80pct_of_permuted": dss.get("num_permuted_worse_than_real", 0) >= 0.8 * N_PERM_REPEATS,
    "all_permuted_pvs": dss.get("all_permuted_pvs", []),
}
# signed_max_abs
dsm = perm_dist_signed_max_stats
diag_p1["signed_max_abs"] = {
    "real_total_pv": dsm["real_total_pv"],
    "permuted_min_total_pv": dsm["min_total_pv"],
    "permuted_max_total_pv": dsm["max_total_pv"],
    "permuted_mean_total_pv": dsm["mean_total_pv"],
    "permuted_median_total_pv": dsm["median_total_pv"],
    "permuted_std_total_pv": dsm["std_total_pv"],
    "num_permuted_better_than_real": dsm["num_permuted_better_than_real"],
    "num_permuted_equal_to_real": dsm.get("num_permuted_equal_to_real", 0),
    "num_permuted_worse_than_real": dsm["num_permuted_worse_than_real"],
    "real_percentile_against_permuted": dsm["real_percentile_against_permuted"],
    "real_beats_permuted_median": dsm["real_total_pv"] < dsm["median_total_pv"],
    "real_beats_80pct_of_permuted": dsm.get("num_permuted_worse_than_real", 0) >= 0.8 * N_PERM_REPEATS,
    "all_permuted_pvs": dsm.get("all_permuted_pvs", []),
}
high_ceiling_gap_diagnostic["part1_permutation_distribution"] = diag_p1

# ---------------------------------------------------------------------------
# Part 2: Online vs offline key coverage
# ---------------------------------------------------------------------------
# Collect all keys with nonzero signed weights from online and offline memories
def get_all_signed_keys(memory_obj):
    """Return {key: weight} for all keys with nonzero signed weight."""
    keys = {}
    for key in memory_obj.counts:
        w_raw = memory_obj.get_raw_risk_weight_signed(*key)
        if abs(w_raw) > 1e-10:
            keys[key] = w_raw
    return keys

online_keys_all = get_all_signed_keys(mem_signed_sum)
offline_keys_all = get_all_signed_keys(fcrm_offline)

online_positive_keys = {k: w for k, w in online_keys_all.items() if w > 0}
online_negative_keys = {k: w for k, w in online_keys_all.items() if w < 0}
offline_positive_keys = {k: w for k, w in offline_keys_all.items() if w > 0}
offline_negative_keys = {k: w for k, w in offline_keys_all.items() if w < 0}

# Top 10 by abs weight
def top_n_by_abs(d, n=10):
    return sorted(d.items(), key=lambda x: abs(x[1]), reverse=True)[:n]

online_top10_pos = set(k for k, _ in top_n_by_abs(online_positive_keys, 10))
online_top10_neg = set(k for k, _ in top_n_by_abs(online_negative_keys, 10))
offline_top10_pos = set(k for k, _ in top_n_by_abs(offline_positive_keys, 10))
offline_top10_neg = set(k for k, _ in top_n_by_abs(offline_negative_keys, 10))

overlap_top10_pos = online_top10_pos & offline_top10_pos
overlap_top10_neg = online_top10_neg & offline_top10_neg
offline_only_pos = offline_top10_pos - online_top10_pos
offline_only_neg = offline_top10_neg - online_top10_neg
online_only_pos = online_top10_pos - offline_top10_pos
online_only_neg = online_top10_neg - offline_top10_neg

# Determine which offline-only keys exist due to not-observed vs below-support
missing_offline_keys_not_observed = []
missing_offline_keys_below_support = []
for key in (offline_only_pos | offline_only_neg):
    goal, action, family, dev_feat = key
    online_stats = mem_signed_sum.counts.get(key)
    if online_stats is None:
        missing_offline_keys_not_observed.append(key)
    else:
        supports = [
            online_stats.get("violation_with", 0),
            online_stats.get("nonviolation_with", 0),
            online_stats.get("violation_without", 0),
            online_stats.get("nonviolation_without", 0),
        ]
        total_support = sum(supports)
        if total_support < MIN_VIOLATION_SUPPORT:
            missing_offline_keys_below_support.append(key)

diag_p2 = {
    "online_supported_positive_keys": len(online_positive_keys),
    "online_supported_negative_keys": len(online_negative_keys),
    "offline_supported_positive_keys": len(offline_positive_keys),
    "offline_supported_negative_keys": len(offline_negative_keys),
    "overlap_top_10_positive_keys": len(overlap_top10_pos),
    "overlap_top_10_negative_keys": len(overlap_top10_neg),
    "offline_only_positive_keys": len(offline_only_pos),
    "offline_only_negative_keys": len(offline_only_neg),
    "online_only_positive_keys": len(online_only_pos),
    "online_only_negative_keys": len(online_only_neg),
    "offline_only_positive_key_list": [list(k) for k in offline_only_pos],
    "offline_only_negative_key_list": [list(k) for k in offline_only_neg],
    "missing_offline_keys_due_to_not_observed_online": len(missing_offline_keys_not_observed),
    "missing_offline_keys_due_to_below_support_online": len(missing_offline_keys_below_support),
    "missing_not_observed_list": [list(k) for k in missing_offline_keys_not_observed],
    "missing_below_support_list": [list(k) for k in missing_offline_keys_below_support],
}
high_ceiling_gap_diagnostic["part2_key_coverage"] = diag_p2

# ---------------------------------------------------------------------------
# Part 3: Evidence acquisition timing
# ---------------------------------------------------------------------------
# Track memory state before/after each episode
learning_curve = part2.get("learning_curve", [])
part3_timeline = []

# Track memory keys over time by snapshotting after each episode
# (memory is cumulatively updated across episodes 0..4)
mem_snapshots = []
# Reconstruct from update_log: keys observed per episode
ep_memory_keys = {ep: set() for ep in range(N_EPISODES)}
for entry in mem_signed_sum.update_log:
    ep_idx = entry.get("episode", -1)
    if 0 <= ep_idx < N_EPISODES:
        key = (entry.get("goal"), entry.get("action"), entry.get("family"), entry.get("dev_feature"))
        ep_memory_keys[ep_idx].add(key)

# Count positive/negative keys after each episode
cumulative_keys = set()
for ep in range(N_EPISODES):
    cumulative_keys |= ep_memory_keys[ep]
    pos_keys = {k for k in cumulative_keys if get_all_signed_keys(mem_signed_sum).get(k, 0) > 0}
    neg_keys = {k for k in cumulative_keys if get_all_signed_keys(mem_signed_sum).get(k, 0) < 0}
    # But get_all_signed_keys reflects FINAL state, not per-episode.
    # Instead, count keys that have any support after this episode.
    mem_snapshots.append({
        "episode": ep,
        "num_memory_keys": len(cumulative_keys),
        "new_keys_this_episode": len(ep_memory_keys[ep]),
    })

# Determine key counts using the learning curve data (which has pos/neg key counts per episode)
part3_timeline = []
for ep_data in learning_curve:
    ep_idx = ep_data["episode_id"]
    a_pv_ep = ep_data["A_total_pv_episode"]
    ss_pv_ep = ep_data.get("B_signed_sum_online_pv_episode", 0)
    pos_keys = ep_data.get("num_positive_weight_keys", 0)
    neg_keys = ep_data.get("num_negative_weight_keys", 0)
    part3_timeline.append({
        "episode_id": ep_idx,
        "A_pv": a_pv_ep,
        "B_signed_sum_online_pv": ss_pv_ep,
        "num_positive_weight_keys": pos_keys,
        "num_negative_weight_keys": neg_keys,
        "signed_sum_pv_reduction_vs_A_episode": a_pv_ep - ss_pv_ep,
    })

# Find first episode where a useful key appears
# "useful" = negative key with abs(weight) above some threshold
first_useful_negative_ep = None
first_useful_positive_ep = None
for ep_data in learning_curve:
    if ep_data.get("num_negative_weight_keys", 0) > 0 and first_useful_negative_ep is None:
        first_useful_negative_ep = ep_data["episode_id"]
    if ep_data.get("num_positive_weight_keys", 0) > 0 and first_useful_positive_ep is None:
        first_useful_positive_ep = ep_data["episode_id"]
    if first_useful_negative_ep is not None and first_useful_positive_ep is not None:
        break

first_useful_ep = None
if first_useful_negative_ep is not None:
    first_useful_ep = first_useful_negative_ep
elif first_useful_positive_ep is not None:
    first_useful_ep = first_useful_positive_ep

eps_remaining = (N_EPISODES - 1) - first_useful_ep if first_useful_ep is not None else 0
evidence_too_late = (
    (first_useful_ep is not None and first_useful_ep > 3)
    or eps_remaining <= 1
)

diag_p3 = {
    "timeline": part3_timeline,
    "first_episode_where_useful_negative_key_appears": first_useful_negative_ep,
    "first_episode_where_useful_positive_key_appears": first_useful_positive_ep,
    "episodes_remaining_after_first_useful_key": eps_remaining,
    "evidence_arrives_too_late": evidence_too_late,
}
high_ceiling_gap_diagnostic["part3_evidence_timing"] = diag_p3

# ---------------------------------------------------------------------------
# Part 4: Selection-difference audit (A vs B_signed_sum)
# ---------------------------------------------------------------------------
part4_audit = []
avoided_pv = 0; avoided_nv = 0; avoided_eff = 0
helpful_count = 0; harmful_count = 0

# Collect A probes: {(oid, action): A_detail}
a_probes = {}
for ep_a in all_episodes_a:
    for d in ep_a["probe_details"]:
        a_probes[(d["oid"], d["action"])] = d

# Collect B probes
b_probes = {}
for ep_b in eps_signed_sum:
    for d in ep_b["probe_details"]:
        b_probes[(d["oid"], d["action"])] = d

# Find probes selected by A but NOT by B_signed_sum
for (oid, action), a_detail in a_probes.items():
    if (oid, action) not in b_probes:
        # A selected this but B did not
        b_detail = b_probes.get((oid, action))
        is_pv = a_detail.get("is_violation", False)
        is_nv = a_detail.get("is_nonviolation", False)
        is_eff = a_detail.get("probe_utility_signal") == "effective"

        if is_pv: avoided_pv += 1
        if is_nv: avoided_nv += 1
        if is_eff: avoided_eff += 1
        if is_pv: helpful_count += 1
        if is_nv: harmful_count += 1

        part4_audit.append({
            "episode_id": a_detail.get("episode", "?"),
            "object_id": oid,
            "action": action,
            "inferred_family": a_detail.get("family", ""),
            "injected_deviation_features": a_detail.get("injected_deviation_present", []),
            "soft_prior": a_detail.get("soft_prior", 0),
            "final_score_A": a_detail.get("final_score", 0),
            "final_score_B": None,  # B didn't probe this
            "prior_violation": is_pv,
            "prior_nonviolation": is_nv,
            "probe_utility_signal": a_detail.get("probe_utility_signal", ""),
            "helped_or_hurt_pv": "helped" if is_pv else "hurt" if is_nv else "neutral",
        })

# Also count probes where B selected but A did not (new selections by B)
# These are cases where signed risk caused B to probe something A didn't
for (oid, action), b_detail in b_probes.items():
    if (oid, action) not in a_probes:
        is_pv = b_detail.get("is_violation", False)
        is_nv = b_detail.get("is_nonviolation", False)
        part4_audit.append({
            "episode_id": b_detail.get("episode", "?"),
            "object_id": oid,
            "action": action,
            "inferred_family": b_detail.get("family", ""),
            "injected_deviation_features": b_detail.get("injected_deviation_present", []),
            "soft_prior": b_detail.get("soft_prior", 0),
            "final_score_A": None,  # A didn't probe this
            "final_score_B": b_detail.get("final_score", 0),
            "signed_risk_penalty": b_detail.get("risk_penalty", 0),
            "prior_violation": is_pv,
            "prior_nonviolation": is_nv,
            "probe_utility_signal": b_detail.get("probe_utility_signal", ""),
            "helped_or_hurt_pv": "helped" if is_pv else "hurt" if is_nv else "neutral",
            "note": "B selected, A did not",
        })
        if is_pv: avoided_pv += 1
        if is_nv: avoided_nv += 1
...
diag_p4 = {
    ...
    "avoided_nonviolation_count": avoided_nv,
    "avoided_effective_probe_count": avoided_eff,
    "helpful_avoidance_count": helpful_count,
    "harmful_avoidance_count": harmful_count,
    "net_helpful_minus_harmful": helpful_count - harmful_count,
    "details": part4_audit[:50],  # Cap to avoid massive output
    "total_audit_entries": len(part4_audit),
}
high_ceiling_gap_diagnostic["part4_selection_audit"] = diag_p4

# ---------------------------------------------------------------------------
# Part 5: Missed offline-ceiling opportunities
# ---------------------------------------------------------------------------
# Compare offline vs online: where would offline avoid a PV but online didn't?
...
missed_wrong_sign = 0
missed_too_weak = 0
missed_selection_order = 0

# Build lookup: for each (oid, action) probed by offline, what was the outcome?
offline_probes = {}
for ep_off in all_eps_offline:
    for d in ep_off["probe_details"]:
        offline_probes[(d["oid"], d["action"])] = d

online_probes_lookup = {}
for ep_on in eps_signed_sum:
    for d in ep_on["probe_details"]:
        online_probes_lookup[(d["oid"], d["action"])] = d

# Find probes where offline detected a PV but online probed it anyway
a_pv_set = set()
for ep_a in all_episodes_a:
    for d in ep_a["probe_details"]:
...
for (oid, action), off_detail in offline_probes.items():
    is_offline_pv = off_detail.get("is_violation", False)
    # Offline avoided this PV (not in offline_probes as PV)
    # Actually: offline probes the object. If offline selected it AND it's a PV,
    # then offline couldn't avoid it either. We want cases where:
    # - A probed and hit a PV: (oid, action) in a_pv_set
    # - Online probed and hit a PV: (oid, action) in online_probes_lookup AND is_violation
    # - Offline did NOT probe, OR offline probed and it was NOT a PV
    pass  # Complex logic - use the simpler approach below

# Simpler approach: For each PV that A committed, check if offline would avoid it
# and online didn't.
for (oid, action) in a_pv_set:
    off_d = offline_probes.get((oid, action))
    on_d = online_probes_lookup.get((oid, action))

    if off_d is None and on_d is None:
        continue  # Neither offline nor online probed this
    if on_d is None:
        continue  # Online avoided it too (not a missed opportunity)

    # Online did probe this PV
    on_is_pv = on_d.get("is_violation", False)
    if not on_is_pv:
        continue  # Online probed but it wasn't a PV for online

    # Offline would NOT have probed this (off_d is None = offline avoided it)
    # OR offline probed but it wouldn't be a PV
    off_would_probe = off_d is not None
    off_would_be_pv = off_d.get("is_violation", False) if off_d is not None else False

    if off_would_probe and off_would_be_pv:
        continue  # Offline also couldn't avoid this

    # This is a missed opportunity: offline avoids, online doesn't
    missed_pv_count += 1
    family = on_d.get("family", "")
    dev_feats = on_d.get("injected_deviation_present", [])
    ...
    for df in dev_feats:
        ...
        on_key_stats = mem_signed_sum.counts.get(key)
        off_key_stats = fcrm_offline.counts.get(key)

        if on_key_stats is None:
            reason = "missing_key"
            missed_missing_key += 1
            break
        elif sum(on_key_stats.values()) < MIN_VIOLATION_SUPPORT:
            reason = "below_support"
            missed_below_support += 1
            break
        else:
            on_weight = mem_signed_sum.get_raw_risk_weight_signed(*key)
            off_weight = fcrm_offline.get_raw_risk_weight_signed(*key) if off_key_stats else 0
            if on_weight > 0 and off_weight < 0:
                reason = "wrong_sign"
                missed_wrong_sign += 1
                break
            elif abs(on_weight) < 0.05:
                reason = "too_weak"
                missed_too_weak += 1
                break
            elif abs(on_weight) >= 0.05:
                reason = "selection_order_not_changed"
                missed_selection_order += 1
                break
    ...
    missed_opportunities.append({
        ...
        "action": action,
        "inferred_family": family,
        "injected_deviation_features": dev_feats,
        "reason": reason,
    })

diag_p5 = {
    "missed_offline_pv_avoidance_count": missed_pv_count,
    "missed_due_to_missing_key": missed_missing_key,
    "missed_due_to_below_support": missed_below_support,
    "missed_due_to_wrong_sign": missed_wrong_sign,
    "missed_due_to_too_weak": missed_too_weak,
    "missed_due_to_selection_order_not_changed": missed_selection_order,
    "details": missed_opportunities[:30],
    "total_missed": len(missed_opportunities),
}
high_ceiling_gap_diagnostic["part5_missed_offline_opportunities"] = diag_p5

# Legacy compatibility: compute indicator variables used by previous blocks
permuted_range = dss["max_total_pv"] - dss["min_total_pv"]
high_offline_floor = offline_pv >= 8
tight_permuted_dist = permuted_range <= 5
real_near_permuted_median = signed_sum_pv <= dss["median_total_pv"] + 1
signed_sum_vs_signed_max_pv_divergence = signed_max_pv - signed_sum_pv

# Part 6: Boolean flags for seed 109 high-ceiling gap diagnostic
ss_beats_a_flag = signed_sum_pv < a_total_pv
ss_beats_perm_median_100 = diag_p1["signed_sum"]["real_beats_permuted_median"]
ss_beats_80pct = diag_p1["signed_sum"]["real_beats_80pct_of_permuted"]
high_ceiling_confirmed = (a_total_pv - offline_pv) >= 12
online_offline_gap_large = (signed_sum_pv - offline_pv) >= 10
mapping_advantage_absent = signed_sum_pv >= dss["median_total_pv"]
key_coverage_insufficient = (
    (diag_p2["offline_only_positive_keys"] + diag_p2["offline_only_negative_keys"])
    > (diag_p2["overlap_top_10_positive_keys"] + diag_p2["overlap_top_10_negative_keys"])
)
weight_signal_wrong_or_weak = (
    (diag_p5["missed_due_to_wrong_sign"] + diag_p5["missed_due_to_too_weak"]
     + diag_p5["missed_due_to_selection_order_not_changed"])
    > (diag_p5["missed_due_to_missing_key"] + diag_p5["missed_due_to_below_support"])
)
evidence_too_late_flag = diag_p3["evidence_arrives_too_late"]

# Determine implementation status
all_sanity_ok = (
    shared_positions_used
    and permuted_control_mode == "online_stepwise"
    and not final_memory_permutation_used
    and deterministic_permutation_seed_used
    and no_oracle_leakage_confirmed
    and not hard_exclusion_used
    and deceptive_family_preservation_valid
)

if all_sanity_ok and ss_beats_a_flag:
    implementation_status = "pass" if ss_beats_perm_median_100 else "partial"
    failure_reason = "none" if ss_beats_perm_median_100 else "real_does_not_beat_permuted_median"
elif not all_sanity_ok:
    implementation_status = "fail"
    failure_reason = "sanity_check_failed"
else:
    implementation_status = "partial"
    failure_reason = "signed_sum_does_not_beat_A"

boolean_flag_details = {
    "seed109_real_beats_A": {
        "value": ss_beats_a_flag,
        "rule": "B_signed_sum_online_total_pv < A_total_pv",
        "supporting_values": {"signed_sum_pv": signed_sum_pv, "A_pv": a_total_pv},
    },
    "seed109_real_beats_permuted_median_100repeat": {
        "value": ss_beats_perm_median_100,
        "rule": "real signed_sum PV < permuted median PV (100 repeats)",
        "supporting_values": {
            "real_pv": signed_sum_pv,
            "permuted_median": dss["median_total_pv"],
            "n_perm_repeats": N_PERM_REPEATS,
        },
    },
    "seed109_real_beats_80pct_permuted": {
        "value": ss_beats_80pct,
        "rule": "real PV beats >= 80% of 100 permuted repeats",
        "supporting_values": {
            "num_worse": dss["num_permuted_worse_than_real"],
            "num_equal": dss.get("num_permuted_equal_to_real", 0),
            "n_total": N_PERM_REPEATS,
        },
    },
    "seed109_high_ceiling_confirmed": {
        "value": high_ceiling_confirmed,
        "rule": "A_total_pv - Offline_signed_sum_upper_bound_total_pv >= 12",
        "supporting_values": {
            "A_pv": a_total_pv,
            "offline_pv": offline_pv,
            "improvement_room": a_total_pv - offline_pv,
        },
    },
    "seed109_online_offline_gap_large": {
        "value": online_offline_gap_large,
        "rule": "B_signed_sum_online_total_pv - Offline_signed_sum_upper_bound_total_pv >= 10",
        "supporting_values": {
            "signed_sum_pv": signed_sum_pv,
            "offline_pv": offline_pv,
            "gap": signed_sum_pv - offline_pv,
        },
    },
    "seed109_mapping_advantage_absent": {
        "value": mapping_advantage_absent,
    ...
    "seed109_online_key_coverage_insufficient": {
        "value": key_coverage_insufficient,
        "rule": "offline_only_pos + offline_only_neg > overlap_top10_pos + overlap_top10_neg",
        "supporting_values": {
            "offline_only_pos": diag_p2["offline_only_positive_keys"],
            "offline_only_neg": diag_p2["offline_only_negative_keys"],
            "overlap_top10_pos": diag_p2["overlap_top_10_positive_keys"],
            "overlap_top10_neg": diag_p2["overlap_top_10_negative_keys"],
        },
    },
    "seed109_evidence_arrives_too_late": {
        "value": evidence_too_late_flag,
        "rule": "first useful key appears after episode 3 or episodes remaining <= 1",
        "supporting_values": {
            "first_useful_neg_ep": diag_p3["first_episode_where_useful_negative_key_appears"],
            "first_useful_pos_ep": diag_p3["first_episode_where_useful_positive_key_appears"],
            "episodes_remaining": diag_p3["episodes_remaining_after_first_useful_key"],
        },
    },
    "seed109_weight_signal_wrong_or_too_weak": {
        "value": weight_signal_wrong_or_weak,
        "rule": "wrong_sign + too_weak + selection_order > missing_key + below_support",
        "supporting_values": {
            "wrong_sign": diag_p5["missed_due_to_wrong_sign"],
            "too_weak": diag_p5["missed_due_to_too_weak"],
            "selection_order": diag_p5["missed_due_to_selection_order_not_changed"],
            "missing_key": diag_p5["missed_due_to_missing_key"],
            "below_support": diag_p5["missed_due_to_below_support"],
        },
    },
    "no_oracle_leakage_confirmed": {
        "value": no_oracle_leakage_confirmed,
        "rule": "all online variants use no offline/oracle evidence",
        "supporting_values": {"all_online_variants_clean": no_oracle_leakage_confirmed},
    },
    "shared_positions_used": {
        "value": shared_positions_used,
        "rule": "all variants use the same precomputed episode positions",
        "supporting_values": {
            "position_hash_by_episode": episode_position_hashes,
        },
    },
    "permuted_control_mode_is_online_stepwise": {
        "value": permuted_control_mode == "online_stepwise",
    ...
    "final_memory_permutation_used": {
        "value": final_memory_permutation_used,
    ...
    "implementation_status": {
        ...
        "supporting_values": {
            "sanity_ok": all_sanity_ok,
            "ss_beats_A": ss_beats_a_flag,
            "ss_beats_perm_median": ss_beats_perm_median_100,
        },
    },
    "failure_reason": {
...
output = {
    ...
    "part4_permutation_distribution": part4,
    ...
    "high_ceiling_gap_diagnostic": high_ceiling_gap_diagnostic,
    "elapsed_seconds": round(elapsed, 1),
}

out_json_path = os.path.join(CURRENT_DIR, "runs",
    f"block1j28_seed109_high_ceiling_gap_diagnostic.json")
...
md_lines = []
md_lines.append("# Block 1J28 -- Seed 109 Weak-Seed Diagnostic for Online Signed Memory")
md_lines.append("")
...
md_lines.append("")
md_lines.append("**This is a deep-dive diagnostic on seed 109**, identified as a weak seed")
md_lines.append("in the 1J25 multiseed robustness test (signed_sum PV=18 ties with permuted")
md_lines.append("median=18, vs seeds 101/107 where signed_sum strongly beats permuted).")
md_lines.append("")
...
md_lines.append("")
md_lines.append("| Variant | Total PV | Dec PV | Norm PV | Macro BAcc | Delta vs A | Eff Probes | Avoided Eff | Avoided PV | Avoided NV | Helpful | Harmful | Hard Excl | Oracle Evid | True Dec Label | Future Outcome | Same-Ep Future |")
md_lines.append("|---------|----------|--------|---------|------------|------------|------------|-------------|------------|------------|---------|---------|-----------|-------------|----------------|----------------|----------------|")
...
# Part 4
md_lines.append("## 7. Part 4: Permutation Distribution (Fix 2: online_stepwise)")
md_lines.append("")
...
md_lines.append(f"- deterministic_permutation_seed_used: {deterministic_permutation_seed_used}")
md_lines.append(f"- n_permutation_repeats: {N_PERM_REPEATS}")
md_lines.append("")
md_lines.append("### Signed Sum Permuted Distribution")
md_lines.append("")
dss = perm_dist_signed_sum_stats
md_lines.append(f"- min_pv: {dss['min_total_pv']}, max_pv: {dss['max_total_pv']}, mean_pv: {dss['mean_total_pv']}, median_pv: {dss['median_total_pv']}, std_pv: {dss['std_total_pv']}")
md_lines.append(f"- real_pv: {dss['real_total_pv']}, real_percentile: {dss['real_percentile_against_permuted']}")
md_lines.append(f"- num_better_than_real: {dss['num_permuted_better_than_real']}, num_equal: {dss['num_permuted_equal_to_real']}, num_worse: {dss['num_permuted_worse_than_real']}")
md_lines.append(f"- all_permuted_pvs: {dss['all_permuted_pvs']}")
md_lines.append("")
md_lines.append("### Signed Max Abs Permuted Distribution")
md_lines.append("")
dsm = perm_dist_signed_max_stats
md_lines.append(f"- min_pv: {dsm['min_total_pv']}, max_pv: {dsm['max_total_pv']}, mean_pv: {dsm['mean_total_pv']}, median_pv: {dsm['median_total_pv']}, std_pv: {dsm['std_total_pv']}")
md_lines.append(f"- real_pv: {dsm['real_total_pv']}, real_percentile: {dsm['real_percentile_against_permuted']}")
md_lines.append(f"- num_better_than_real: {dsm['num_permuted_better_than_real']}, num_equal: {dsm['num_permuted_equal_to_real']}, num_worse: {dsm['num_permuted_worse_than_real']}")
md_lines.append(f"- all_permuted_pvs: {dsm['all_permuted_pvs']}")
md_lines.append("")
md_lines.append("### Single Repeat (r00) Comparison")
md_lines.append("")
md_lines.append("| Real Variant | Real PV | Permuted r00 PV | Real BAcc | Permuted r00 BAcc | Real Beats Permuted r00 |")
md_lines.append("|-------------|---------|-----------------|-----------|-------------------|------------------------|")
for row in part4["single_permuted_comparison"]:
    md_lines.append(f"| {row['real_variant_name']} | {row['real_total_pv']} | "
                   f"{row['permuted_r00_total_pv']} | "
                   f"{row['real_macro_bal']:.4f} | {row['permuted_r00_macro_bal']:.4f} | "
                   f"{row['real_beats_permuted_r00']} |")
md_lines.append("")
...
# Part 7: High-Ceiling Online/Offline Gap Diagnostic (seed 109 specific)
md_lines.append("## 10. Part 7: High-Ceiling Online/Offline Gap Diagnostic")
md_lines.append("")
md_lines.append("Diagnose why seed109 has high offline improvement room (23)")
md_lines.append("but weak online signed_sum improvement (PV=27 vs A=29, reduction of only 2).")
md_lines.append("")

# 10.1 Permutation distribution
md_lines.append("### 10.1 Permutation Distribution (100 repeats)")
md_lines.append("")
md_lines.append("#### Signed Sum")
md_lines.append("")
p1ss = diag_p1["signed_sum"]
md_lines.append(f"- real_total_pv: {p1ss['real_total_pv']}")
md_lines.append(f"- permuted: min={p1ss['permuted_min_total_pv']}, max={p1ss['permuted_max_total_pv']}, "
               f"mean={p1ss['permuted_mean_total_pv']}, median={p1ss['permuted_median_total_pv']}, "
               f"std={p1ss['permuted_std_total_pv']}")
md_lines.append(f"- num_better_than_real: {p1ss['num_permuted_better_than_real']}, "
               f"num_equal: {p1ss['num_permuted_equal_to_real']}, "
               f"num_worse: {p1ss['num_permuted_worse_than_real']}")
md_lines.append(f"- real_percentile: {p1ss['real_percentile_against_permuted']}")
md_lines.append(f"- real_beats_permuted_median: {p1ss['real_beats_permuted_median']}")
md_lines.append(f"- real_beats_80pct: {p1ss['real_beats_80pct_of_permuted']}")
md_lines.append(f"- all_permuted_pvs: {p1ss['all_permuted_pvs']}")
md_lines.append("")
md_lines.append("#### Signed Max Abs")
md_lines.append("")
p1sm = diag_p1["signed_max_abs"]
md_lines.append(f"- real_total_pv: {p1sm['real_total_pv']}")
md_lines.append(f"- permuted: min={p1sm['permuted_min_total_pv']}, max={p1sm['permuted_max_total_pv']}, "
               f"mean={p1sm['permuted_mean_total_pv']}, median={p1sm['permuted_median_total_pv']}, "
               f"std={p1sm['permuted_std_total_pv']}")
md_lines.append(f"- num_better_than_real: {p1sm['num_permuted_better_than_real']}, "
               f"num_equal: {p1sm['num_permuted_equal_to_real']}, "
               f"num_worse: {p1sm['num_permuted_worse_than_real']}")
md_lines.append(f"- real_percentile: {p1sm['real_percentile_against_permuted']}")
md_lines.append(f"- real_beats_permuted_median: {p1sm['real_beats_permuted_median']}")
md_lines.append(f"- real_beats_80pct: {p1sm['real_beats_80pct_of_permuted']}")
md_lines.append("")

# 10.2 Key coverage
md_lines.append("### 10.2 Online vs Offline Key Coverage")
md_lines.append("")
p2 = diag_p2
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| online_supported_positive_keys | {p2['online_supported_positive_keys']} |")
md_lines.append(f"| online_supported_negative_keys | {p2['online_supported_negative_keys']} |")
md_lines.append(f"| offline_supported_positive_keys | {p2['offline_supported_positive_keys']} |")
md_lines.append(f"| offline_supported_negative_keys | {p2['offline_supported_negative_keys']} |")
md_lines.append(f"| overlap_top_10_positive_keys | {p2['overlap_top_10_positive_keys']} |")
md_lines.append(f"| overlap_top_10_negative_keys | {p2['overlap_top_10_negative_keys']} |")
md_lines.append(f"| offline_only_positive_keys | {p2['offline_only_positive_keys']} |")
md_lines.append(f"| offline_only_negative_keys | {p2['offline_only_negative_keys']} |")
md_lines.append(f"| online_only_positive_keys | {p2['online_only_positive_keys']} |")
md_lines.append(f"| online_only_negative_keys | {p2['online_only_negative_keys']} |")
md_lines.append(f"| missing_offline_keys_not_observed | {p2['missing_offline_keys_due_to_not_observed_online']} |")
md_lines.append(f"| missing_offline_keys_below_support | {p2['missing_offline_keys_due_to_below_support_online']} |")
if p2.get("offline_only_positive_key_list"):
    md_lines.append(f"- offline_only_positive_keys: {p2['offline_only_positive_key_list']}")
if p2.get("offline_only_negative_key_list"):
    md_lines.append(f"- offline_only_negative_keys: {p2['offline_only_negative_key_list']}")
md_lines.append("")

# 10.3 Evidence timing
md_lines.append("### 10.3 Evidence Acquisition Timing")
md_lines.append("")
p3 = diag_p3
md_lines.append("| Ep | A PV | SS PV | PosKeys | NegKeys | Reduction vs A |")
md_lines.append("|----|------|-------|---------|---------|----------------|")
for t in p3["timeline"]:
    md_lines.append(f"| {t['episode_id']} | {t['A_pv']} | {t['B_signed_sum_online_pv']} | "
                   f"{t['num_positive_weight_keys']} | {t['num_negative_weight_keys']} | "
                   f"{t['signed_sum_pv_reduction_vs_A_episode']} |")
md_lines.append("")
md_lines.append(f"- first_episode_where_useful_negative_key_appears: {p3['first_episode_where_useful_negative_key_appears']}")
md_lines.append(f"- first_episode_where_useful_positive_key_appears: {p3['first_episode_where_useful_positive_key_appears']}")
md_lines.append(f"- episodes_remaining_after_first_useful_key: {p3['episodes_remaining_after_first_useful_key']}")
md_lines.append(f"- evidence_arrives_too_late: {p3['evidence_arrives_too_late']}")
md_lines.append("")

# 10.4 Selection audit
md_lines.append("### 10.4 Selection-Difference Audit (A vs B_signed_sum)")
md_lines.append("")
p4 = diag_p4
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| avoided_prior_violation_count | {p4['avoided_prior_violation_count']} |")
md_lines.append(f"| avoided_nonviolation_count | {p4['avoided_nonviolation_count']} |")
md_lines.append(f"| avoided_effective_probe_count | {p4['avoided_effective_probe_count']} |")
md_lines.append(f"| helpful_avoidance_count | {p4['helpful_avoidance_count']} |")
md_lines.append(f"| harmful_avoidance_count | {p4['harmful_avoidance_count']} |")
md_lines.append(f"| net_helpful_minus_harmful | {p4['net_helpful_minus_harmful']} |")
md_lines.append(f"| total_audit_entries | {p4['total_audit_entries']} |")
if p4.get("details"):
    md_lines.append("")
    md_lines.append("| OID | Action | Family | Dev Feats | A PV | A NV | Helped/Hurt |")
    md_lines.append("|-----|--------|--------|------------|------|------|-------------|")
    for d in p4["details"][:20]:
        md_lines.append(f"| {d['object_id']} | {d['action']} | {d['inferred_family']} | "
                       f"{d['injected_deviation_features']} | "
                       f"{d['prior_violation']} | {d['prior_nonviolation']} | "
                       f"{d['helped_or_hurt_pv']} |")
md_lines.append("")

# 10.5 Missed opportunities
md_lines.append("### 10.5 Missed Offline-Ceiling Opportunities")
md_lines.append("")
p5 = diag_p5
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| missed_offline_pv_avoidance_count | {p5['missed_offline_pv_avoidance_count']} |")
md_lines.append(f"| missed_due_to_missing_key | {p5['missed_due_to_missing_key']} |")
md_lines.append(f"| missed_due_to_below_support | {p5['missed_due_to_below_support']} |")
md_lines.append(f"| missed_due_to_wrong_sign | {p5['missed_due_to_wrong_sign']} |")
md_lines.append(f"| missed_due_to_too_weak | {p5['missed_due_to_too_weak']} |")
md_lines.append(f"| missed_due_to_selection_order | {p5['missed_due_to_selection_order_not_changed']} |")
md_lines.append(f"| total_missed | {p5['total_missed']} |")
if p5.get("details"):
    md_lines.append("")
    md_lines.append("| OID | Action | Family | Dev Feats | Reason |")
    md_lines.append("|-----|--------|--------|------------|--------|")
    for d in p5["details"][:20]:
        md_lines.append(f"| {d['object_id']} | {d['action']} | {d['inferred_family']} | "
                       f"{d['injected_deviation_features']} | {d['reason']} |")
md_lines.append("")
...
md_lines.append(f"block_id=1J28")
md_lines.append(f"seed=109")
md_lines.append(f"n_permutation_repeats={N_PERM_REPEATS}")
...
md_lines.append(f"signed_sum_permuted_median_pv={dss['median_total_pv']}")
md_lines.append(f"signed_sum_permuted_mean_pv={dss['mean_total_pv']}")
md_lines.append(f"signed_sum_permuted_min_pv={dss['min_total_pv']}")
md_lines.append(f"signed_sum_permuted_max_pv={dss['max_total_pv']}")
md_lines.append(f"signed_sum_num_permuted_better_than_real={dss['num_permuted_better_than_real']}")
md_lines.append(f"signed_sum_num_permuted_equal_to_real={dss.get('num_permuted_equal_to_real', 0)}")
md_lines.append(f"signed_sum_num_permuted_worse_than_real={dss['num_permuted_worse_than_real']}")
md_lines.append(f"signed_sum_real_percentile_against_permuted={dss['real_percentile_against_permuted']}")
md_lines.append(f"seed109_real_beats_A={ss_beats_a_flag}")
md_lines.append(f"seed109_real_beats_permuted_median_100repeat={ss_beats_perm_median_100}")
md_lines.append(f"seed109_real_beats_80pct_permuted={ss_beats_80pct}")
md_lines.append(f"seed109_high_ceiling_confirmed={high_ceiling_confirmed}")
md_lines.append(f"seed109_online_offline_gap_large={online_offline_gap_large}")
md_lines.append(f"seed109_mapping_advantage_absent={mapping_advantage_absent}")
md_lines.append(f"seed109_online_key_coverage_insufficient={key_coverage_insufficient}")
md_lines.append(f"seed109_evidence_arrives_too_late={evidence_too_late_flag}")
md_lines.append(f"seed109_weight_signal_wrong_or_too_weak={weight_signal_wrong_or_weak}")
...
out_md_path = os.path.join(CURRENT_DIR, "protocols",
    f"block1j28_seed109_high_ceiling_gap_diagnostic.md")
...
print("\n" + "=" * 70)
print("Block 1J28 complete.")
print(f"  A PV={a_total_pv}  signed_sum PV={signed_sum_pv}  signed_max PV={signed_max_pv}  offline PV={offline_pv}")
print(f"  signed_sum permuted median={perm_dist_signed_sum_stats['median_total_pv']}  "
      f"signed_max permuted median={perm_dist_signed_max_stats['median_total_pv']}")
print(f"  signed_sum_beats_A={signed_sum_online_beats_A}  "
      f"beats_permuted_median={signed_sum_online_beats_permuted_median}")
print(f"  shared_positions={shared_positions_used}  "
      f"permuted_control_mode={permuted_control_mode}")
print(f"  no_oracle_leakage={no_oracle_leakage_confirmed}")

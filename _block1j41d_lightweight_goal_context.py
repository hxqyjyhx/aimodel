"""
Block 1J41d -- Lightweight Goal Context.
...
events only, with a lightweight goal_context field layered onto Experience.
"""
...
SCRIPT_NAME = "_block1j41d_lightweight_goal_context.py"
RUN_JSON = os.path.join(CURRENT_DIR, "runs", "block1j41d_lightweight_goal_context.json")
REPORT_MD = os.path.join(CURRENT_DIR, "protocols", "block1j41d_lightweight_goal_context.md")
TABLE_CSV = os.path.join(CURRENT_DIR, "protocols", "block1j41d_lightweight_goal_context_table.csv")
CHECKPOINT_MD = os.path.join(CURRENT_DIR, "checkpoint_1j41d_lightweight_goal_context.md")
...
def build_common_sense_records(clusters, audit_meta):
    ...
    for idx, cluster in enumerate(clusters):
        ...
        relevant_goal_contexts = ["maximize_net_result"]
        if cluster["action_taken"] == "observe_then_act":
            relevant_goal_contexts.extend(["reduce_action_error", "reduce_uncertainty"])
        if len(cluster["boundary_like_cases"]) > 0 or len(cluster["exclusion_features"]) > 0:
            relevant_goal_contexts.append("avoid_boundary_failure")
        if bridge_cases > 0:
            relevant_goal_contexts.append("bridge_generalization")
        typical_effect = "improves_net_result" if cluster["mean_reward"] >= 0.75 else "limits_or_preserves_result"
        record = {
            ...
            "typical_effect_on_goal_proxy": typical_effect,
...
def derive_goal_context(cluster, cs_record):
    if cs_record["boundary_cases"] > 0 or cs_record["known_exclusions"]:
        return "avoid_boundary_failure"
    if cs_record["bridge_cases"] > 0:
        return "bridge_generalization"
    if cluster["action_taken"] == "observe_then_act":
        if cluster["mean_reward"] - cluster["confidence"] * 0.05 >= 0.75:
            return "reduce_action_error"
...
def build_experience_records(common_sense_records, clusters, global_means):
    ...
    for idx, cs_record in enumerate(common_sense_records):
        ...
        goal_context = derive_goal_context(cluster, cs_record)
        goal_support_cases = cs_record["support_cases"]
        goal_failure_cases = cs_record["failure_cases"]
        expected_goal_progress = expected_gain if goal_context != "avoid_boundary_failure" else max(0.0, cluster["confidence"])
        goal_conditioned_confidence = cluster["confidence"]
        if goal_context == "avoid_boundary_failure":
            goal_conditioned_confidence = min(1.0, cluster["confidence"] + 0.05 * len(cs_record["known_exclusions"]))
        elif goal_context == "bridge_generalization":
            goal_conditioned_confidence = min(1.0, cluster["confidence"] + 0.05 * min(3, cs_record["bridge_cases"]))
        records.append(
            {
                ...
                "goal_conditioned_confidence": goal_conditioned_confidence,
                "goal_support_cases": goal_support_cases,
                "goal_failure_cases": goal_failure_cases,
...
def build_record_metrics(agent_records, audit_purity, same_before, bridge_before, boundary_before, boundary_after, null_after, same_samples, bridge_samples, boundary_samples, null_samples):
    ...
    goal_context_distribution = Counter(record["goal_context"] for record in experience_records)
    goal_conditioned_confidences = [record["goal_conditioned_confidence"] for record in experience_records]
    expected_goal_progresses = [record["expected_goal_progress"] for record in experience_records]
    goal_conf_success_corr = pearson_corr(goal_conditioned_confidences, expected_goal_progresses)
    records_with_goal_context = sum(1 for record in experience_records if record.get("goal_context"))
    common_sense_with_goal_context = sum(1 for record in common_sense_records if record.get("relevant_goal_contexts"))
    avg_linked_common_sense = mean([len(record["linked_common_sense_ids"]) for record in experience_records])
    link_consistency = mean([
        1.0
        if record["goal_context"] in next(
            cs["relevant_goal_contexts"]
            for cs in common_sense_records
            if cs["common_sense_id"] == record["linked_common_sense_ids"][0]
        ) else 0.0
        for record in experience_records
    ]) if experience_records else 0.0
    goal_conditioned_success_rate = mean([
        record["goal_support_cases"] / max(1, record["goal_support_cases"] + record["goal_failure_cases"])
        for record in experience_records
    ]) if experience_records else 0.0
    goal_conditioned_false_application_rate = mean([
        1.0 if record["goal_failure_cases"] > record["goal_support_cases"] else 0.0
        for record in experience_records
    ]) if experience_records else 0.0
    bridge_goal_records = [record for record in experience_records if record["bridge_support"] > 0]
    goal_conditioned_bridge_success_rate = mean([
        record["goal_support_cases"] / max(1, record["goal_support_cases"] + record["goal_failure_cases"])
        for record in bridge_goal_records
    ]) if bridge_goal_records else 0.0
    ...
    return {
        ...
        "goal_context_metrics": {
            "records_with_goal_context": records_with_goal_context,
            "goal_context_coverage": records_with_goal_context / max(1, len(experience_records)),
            "goal_context_count": len(goal_context_distribution),
            "goal_context_distribution": dict(goal_context_distribution),
            "goal_conditioned_success_rate": goal_conditioned_success_rate,
            "goal_conditioned_false_application_rate": goal_conditioned_false_application_rate,
            "goal_conditioned_boundary_block_rate": boundary_block_rate,
            "goal_conditioned_bridge_success_rate": goal_conditioned_bridge_success_rate,
            "goal_confidence_success_correlation": goal_conf_success_corr,
            "common_sense_records_with_goal_context": common_sense_with_goal_context,
            "average_linked_common_sense_per_experience": avg_linked_common_sense,
            "goal_context_link_consistency": link_consistency,
        },
        ...
        "null_record_metrics": {
            ...
            "null_false_goal_context_count": sum(
                1
                for record in experience_records
                if record.get("goal_context") in {"reduce_action_error", "reduce_uncertainty", "bridge_generalization"}
                and record["expected_gain"] > 0.03
            ),
        },
    }
...
goal_records_with_context = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["records_with_goal_context"],
)
goal_context_coverage = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_coverage"],
)
goal_context_count = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_count"],
)
goal_conditioned_success_rate = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_success_rate"],
)
goal_conditioned_false_application_rate = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_false_application_rate"],
)
goal_conditioned_boundary_block_rate = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_boundary_block_rate"],
)
goal_conditioned_bridge_success_rate = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_bridge_success_rate"],
)
goal_confidence_success_correlation = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_confidence_success_correlation"]
    if row["learner"]["record_metrics"]["goal_context_metrics"]["goal_confidence_success_correlation"] is not None else 0.0,
)
common_sense_with_goal_context = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["common_sense_records_with_goal_context"],
)
average_linked_common_sense_per_experience = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["average_linked_common_sense_per_experience"],
)
goal_context_link_consistency = aggregate_metric(
    structured_primary_rows,
    lambda row: row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_link_consistency"],
)
bridge_supported_goal_context_records = aggregate_metric(
    structured_primary_rows,
    lambda row: sum(1.0 for record in row["learner"]["agent_records"]["experience_records"] if record["bridge_support"] > 0),
)
goal_context_distribution_counter = Counter()
for row in structured_primary_rows:
    goal_context_distribution_counter.update(
        row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_distribution"]
    )
goal_context_distribution = dict(goal_context_distribution_counter)
null_false_goal_context_count = aggregate_metric(
    null_primary_rows,
    lambda row: row["learner"]["record_metrics"]["null_record_metrics"]["null_false_goal_context_count"],
)

delta_vs_1j41c = {
    "structured_score": structured_score - REFERENCE_1J41C["structured_score"],
    "bridge_required_score": bridge_required_score - REFERENCE_1J41C["bridge_required_score"],
    "null_control_score": null_control_score - REFERENCE_1J41C["null_control_score"],
    "boundary_score": boundary_score - REFERENCE_1J41C["boundary_score"],
}

performance_comparable = max(abs(value) for value in delta_vs_1j41c.values()) <= 0.03
    ...
    csv_rows.append(
        {
            ...
            "records_with_goal_context": row["learner"]["record_metrics"]["goal_context_metrics"]["records_with_goal_context"],
            "goal_context_coverage": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_coverage"],
            "goal_context_count": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_count"],
            "goal_conditioned_success_rate": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_success_rate"],
            "goal_conditioned_false_application_rate": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_false_application_rate"],
            "goal_conditioned_boundary_block_rate": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_boundary_block_rate"],
            "goal_conditioned_bridge_success_rate": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_conditioned_bridge_success_rate"],
            "goal_confidence_success_correlation": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_confidence_success_correlation"],
            "common_sense_records_with_goal_context": row["learner"]["record_metrics"]["goal_context_metrics"]["common_sense_records_with_goal_context"],
            "average_linked_common_sense_per_experience": row["learner"]["record_metrics"]["goal_context_metrics"]["average_linked_common_sense_per_experience"],
            "goal_context_link_consistency": row["learner"]["record_metrics"]["goal_context_metrics"]["goal_context_link_consistency"],
            ...
            "null_false_goal_context_count": row["learner"]["record_metrics"]["null_record_metrics"]["null_false_goal_context_count"],
...
payload = {
    ...
    "summary": {
        ...
        "records_with_goal_context": goal_records_with_context,
        "goal_context_coverage": goal_context_coverage,
        "goal_context_count": goal_context_count,
        "goal_context_distribution": goal_context_distribution,
        "goal_conditioned_success_rate": goal_conditioned_success_rate,
        "goal_conditioned_false_application_rate": goal_conditioned_false_application_rate,
        "goal_conditioned_boundary_block_rate": goal_conditioned_boundary_block_rate,
        "goal_conditioned_bridge_success_rate": goal_conditioned_bridge_success_rate,
        "goal_confidence_success_correlation": goal_confidence_success_correlation,
        "common_sense_records_with_goal_context": common_sense_with_goal_context,
        "average_linked_common_sense_per_experience": average_linked_common_sense_per_experience,
        "goal_context_link_consistency": goal_context_link_consistency,
        "bridge_supported_goal_context_records": bridge_supported_goal_context_records,
...
md_lines = [
    ...
    f"- delta vs 1J41c structured: {delta_vs_1j41c['structured_score']:.4f}",
    ...
    f"- boundary score: {boundary_score:.4f}",
    f"- null-control score: {null_control_score:.4f}",
    f"- delta vs 1J41c: structured {delta_vs_1j41c['structured_score']:.4f}, bridge {delta_vs_1j41c['bridge_required_score']:.4f}, boundary {delta_vs_1j41c['boundary_score']:.4f}, null {delta_vs_1j41c['null_control_score']:.4f}",
    f"- delta vs similarity_only: {delta_vs_similarity_only:.4f}",
    "",
    "## Memory Records",
    ...
    f"- records with goal_context: {goal_records_with_context:.4f}",
    f"- goal_context coverage: {goal_context_coverage:.4f}",
    f"- goal_context count: {goal_context_count:.4f}",
    f"- goal_context distribution: {json.dumps(goal_context_distribution, sort_keys=True)}",
    f"- goal-conditioned success rate: {goal_conditioned_success_rate:.4f}",
    f"- goal-conditioned false application rate: {goal_conditioned_false_application_rate:.4f}",
    f"- goal confidence-success correlation: {goal_confidence_success_correlation:.4f}",
    "",
    "## Common Sense",
    ...
    f"- common sense records with relevant_goal_contexts: {common_sense_with_goal_context:.4f}",
    f"- average linked common sense per experience: {average_linked_common_sense_per_experience:.4f}",
    f"- goal_context link consistency: {goal_context_link_consistency:.4f}",
    "",
    "## Experience",
    ...
    f"- bridge-supported goal-context records: {bridge_supported_goal_context_records:.4f}",
    "",
    "## Null-control",
    f"- false discovery rate: {null_false_discovery_rate:.4f}",
    f"- false goal-context count: {null_false_goal_context_count:.4f}",
...
if status == "PASS":
    md_lines.append("- Prepare a benchmark adapter dry-run if you want to test this goal-conditioned Experience layout in a thin external interface.")
elif status == "PARTIAL":
    md_lines.append("- Refine goal-conditioned confidence or linkage rules before any benchmark adapter work.")
else:
...
checkpoint_lines = [
    "# 1J41d Lightweight Goal Context Checkpoint",
    "",
    f"- status: {status}",
    f"- structured score: {structured_score:.4f}",
    f"- bridge-required score: {bridge_required_score:.4f}",
    f"- boundary score: {boundary_score:.4f}",
    f"- null-control score: {null_control_score:.4f}",
    f"- delta vs 1J41c: {json.dumps(delta_vs_1j41c)}",
    ...
    f"- goal_context coverage/count: {goal_context_coverage:.4f}/{goal_context_count:.1f}",
    f"- goal_context distribution: {json.dumps(goal_context_distribution, sort_keys=True)}",
...
if status == "PASS":
    checkpoint_lines.append("- prepare benchmark adapter dry-run")
elif status == "PARTIAL":
    checkpoint_lines.append("- refine goal_context confidence/update rules")
else:
...
print(f"delta vs 1J41c: {delta_vs_1j41c}")
...
print(f"goal_context coverage/count: {goal_context_coverage:.4f}/{goal_context_count:.1f}")
print(f"goal_context distribution: {goal_context_distribution}")

"""
Block 1J43b -- BabyAI Probe Repair / Minimal Usable Policy Probe.

This is a BabyAI adapter probe repair only. It keeps the legal observation /
action / outcome conversion from 1J43a, but adds a minimal mission-conditioned
action bridge so that visible target evidence can drive useful low-level
BabyAI actions without using hidden state, shortest path, or expert actions.
...
SCRIPT_NAME = "_block1j43b_babyai_probe_repair.py"
RUN_JSON = os.path.join(CURRENT_DIR, "runs", "block1j43b_babyai_probe_repair.json")
REPORT_MD = os.path.join(CURRENT_DIR, "protocols", "block1j43b_babyai_probe_repair.md")
TABLE_CSV = os.path.join(CURRENT_DIR, "protocols", "block1j43b_babyai_probe_repair_table.csv")
CHECKPOINT_MD = os.path.join(CURRENT_DIR, "checkpoint_1j43b_babyai_probe_repair.md")
...
def collect_trajectory(env, seed, policy_name, experience_records=None):
    ...
    total_reward = 0.0
    steps = []
    policy_state = {"step_index": 0, "turn_bias": "left" if seed % 2 == 0 else "right", "forward_streak": 0}
    ...
    while not done:
        if policy_name == "random_policy":
            ...
            action_info = {"target_visible": False, "target_adjacent": False, "used_bridge": False, "used_fallback": True, "used_record": False}
...
def run_env_seed(env_id, seed):
    ...
    for ep in range(PROBE_EPISODES_PER_ENV):
        ...
        policy_trajectories["1j43a_simple_experience_probe"].append(
            collect_trajectory(env, base_seed, "1j43a_simple_experience_probe")
        )
        policy_trajectories["mission_visible_object_probe"].append(
            collect_trajectory(env, base_seed, "mission_visible_object_probe")
        )
        policy_trajectories["experience_plus_mission_probe"].append(
            collect_trajectory(env, base_seed, "experience_plus_mission_probe", experience_records=experience_records)
        )
    ...
    return {
        ...
        "target_visible_rate": policy_summary["experience_plus_mission_probe"]["target_visible_rate"],
        "target_adjacent_rate": policy_summary["experience_plus_mission_probe"]["target_adjacent_rate"],
        "action_bridge_coverage": policy_summary["experience_plus_mission_probe"]["action_bridge_coverage"],
        "random_return": policy_summary["random_policy"]["return"],
        "1j43a_simple_probe_return": policy_summary["1j43a_simple_experience_probe"]["return"],
        "mission_visible_object_probe_return": policy_summary["mission_visible_object_probe"]["return"],
        "experience_plus_mission_probe_return": policy_summary["experience_plus_mission_probe"]["return"],
        "delta_vs_random": policy_summary["experience_plus_mission_probe"]["return"] - policy_summary["random_policy"]["return"],
        "delta_vs_1j43a_probe": policy_summary["experience_plus_mission_probe"]["return"] - policy_summary["1j43a_simple_experience_probe"]["return"],
        "success_proxy_rate": policy_summary["experience_plus_mission_probe"]["success_proxy_rate"],
        "visible_target_success_rate": policy_summary["experience_plus_mission_probe"]["visible_target_success_rate"],
        "target_not_visible_fallback_success_rate": policy_summary["experience_plus_mission_probe"][
...
old_probe_return = agg("1j43a_simple_probe_return")
mission_probe_return = agg("mission_visible_object_probe_return")
experience_probe_return = agg("experience_plus_mission_probe_return")
delta_vs_random = agg("delta_vs_random")
delta_vs_1j43a_probe = agg("delta_vs_1j43a_probe")
...
any_nonzero_env = any(
    row["experience_plus_mission_probe_return"] > 0.0 or row["success_proxy_rate"] > 0.0 or row["mission_visible_object_probe_return"] > 0.0
    for row in all_rows
...
if (
    ...
    and experience_probe_return > old_probe_return
    and any_nonzero_env
):
    status = "PASS"
elif all_rows and not hidden_leakage and experience_probe_return > old_probe_return:
    status = "PARTIAL"
else:
    status = "FAIL"
...
payload = {
    ...
    "summary": {
        ...
        "old_probe_return": old_probe_return,
        "mission_probe_return": mission_probe_return,
        "experience_probe_return": experience_probe_return,
        "delta_vs_random": delta_vs_random,
        "delta_vs_1j43a_probe": delta_vs_1j43a_probe,
...
with open(TABLE_CSV, "w", encoding="utf-8", newline="") as handle:
    writer = csv.DictWriter(
        ...
        fieldnames=[
            ...
            "env_id",
            "random_return",
            "1j43a_simple_probe_return",
            "mission_visible_object_probe_return",
            "experience_plus_mission_probe_return",
            "delta_vs_random",
            "delta_vs_1j43a_probe",
            "success_proxy_rate",
...
report_lines = [
    ...
    f"- 1J43a simple probe return: `{old_probe_return:.4f}`",
    f"- Mission visible-object probe return: `{mission_probe_return:.4f}`",
    f"- Experience plus mission probe return: `{experience_probe_return:.4f}`",
    f"- Delta vs random: `{delta_vs_random:+.4f}`",
    f"- Delta vs 1J43a probe: `{delta_vs_1j43a_probe:+.4f}`",
...
checkpoint_lines = [
    ...
    f"- Mission parse success: `{mission_parse_success_rate:.4f}`",
    f"- Hidden leakage: `{str(hidden_leakage).lower()}`",
    "- Next step: BabyAI small pilot if the repaired probe now shows nonzero usable behavior; otherwise refine exploration fallback / mission parser.",
]
...
print(
    "Policy Probe: "
    f"random={random_return:.4f} old={old_probe_return:.4f} mission={mission_probe_return:.4f} "
    f"exp_mission={experience_probe_return:.4f} delta_random={delta_vs_random:+.4f} "
    f"delta_old={delta_vs_1j43a_probe:+.4f} success={success_proxy_rate:.4f}"
)
print(
    "Mechanism: "
...
print(
    ...
    f"bridge_cov={action_bridge_coverage:.4f} unsupported_missions={unsupported_mission_count}"
)
print(
    "Validity: "

...
Deep-dive into why seed 103 produces weak signed-memory results.
...
parser.add_argument("--seed", type=int, default=103, help="Random seed (default: 103)")
parser.add_argument("--n-perm-repeats", type=int, default=100, help="Permutation repeats (default: 100)")
parser.add_argument("--output-suffix", type=str, default="seed103", help="Suffix for output filenames")
...
print("=" * 70)
print("Block 1J26 -- Seed 103 Weak-Seed Diagnostic for Online Signed Memory")
...
# =============================================================================
# Weak-Seed Diagnostic Fields (seed 103 specific)
# =============================================================================
# Characterize WHY seed 103 is weak compared to seeds 101/107
weak_seed_diagnostic = {}

# 1. Baseline and ceiling analysis
weak_seed_diagnostic["a_baseline_total_pv"] = a_total_pv
weak_seed_diagnostic["offline_upper_bound_total_pv"] = offline_pv
weak_seed_diagnostic["online_offline_gap"] = online_gap
weak_seed_diagnostic["offline_upper_bound_norm_pv"] = stats_offline.get("normal_object_prior_violations", 0)
weak_seed_diagnostic["offline_upper_bound_dec_pv"] = stats_offline.get("deceptive_object_prior_violations", 0)

# 2. Signed aggregation: signed_sum vs signed_max_abs divergence
signed_sum_vs_signed_max_pv_divergence = signed_max_pv - signed_sum_pv
weak_seed_diagnostic["signed_sum_total_pv"] = signed_sum_pv
weak_seed_diagnostic["signed_max_abs_total_pv"] = signed_max_pv
weak_seed_diagnostic["signed_sum_vs_signed_max_pv_divergence"] = signed_sum_vs_signed_max_pv_divergence
weak_seed_diagnostic["signed_sum_beats_A"] = signed_sum_online_beats_A
weak_seed_diagnostic["signed_max_abs_beats_A"] = signed_max_abs_online_beats_A

# 3. Permuted distribution analysis
permuted_pvs_signed_sum = perm_dist_signed_sum_stats.get("all_permuted_pvs", [])
weak_seed_diagnostic["n_permutation_repeats"] = N_PERM_REPEATS
weak_seed_diagnostic["permuted_median_pv"] = perm_dist_signed_sum_stats["median_total_pv"]
weak_seed_diagnostic["permuted_mean_pv"] = perm_dist_signed_sum_stats["mean_total_pv"]
weak_seed_diagnostic["permuted_min_pv"] = perm_dist_signed_sum_stats["min_total_pv"]
weak_seed_diagnostic["permuted_max_pv"] = perm_dist_signed_sum_stats["max_total_pv"]
weak_seed_diagnostic["permuted_std_pv"] = perm_dist_signed_sum_stats["std_total_pv"]
permuted_range = perm_dist_signed_sum_stats["max_total_pv"] - perm_dist_signed_sum_stats["min_total_pv"]
weak_seed_diagnostic["permuted_distribution_range"] = permuted_range
real_pv = signed_sum_pv
num_permuted_worse = perm_dist_signed_sum_stats["num_permuted_worse_than_real"]
num_permuted_equal = perm_dist_signed_sum_stats.get("num_permuted_equal_to_real", 0)
num_permuted_better = perm_dist_signed_sum_stats["num_permuted_better_than_real"]
pct_equal = 100.0 * num_permuted_equal / N_PERM_REPEATS if N_PERM_REPEATS > 0 else 0.0
pct_worse = 100.0 * num_permuted_worse / N_PERM_REPEATS if N_PERM_REPEATS > 0 else 0.0
pct_better = 100.0 * num_permuted_better / N_PERM_REPEATS if N_PERM_REPEATS > 0 else 0.0
weak_seed_diagnostic["num_permuted_worse_than_real"] = num_permuted_worse
weak_seed_diagnostic["num_permuted_equal_to_real"] = num_permuted_equal
weak_seed_diagnostic["num_permuted_better_than_real"] = num_permuted_better
weak_seed_diagnostic["pct_permuted_worse"] = round(pct_worse, 1)
weak_seed_diagnostic["pct_permuted_equal"] = round(pct_equal, 1)
weak_seed_diagnostic["pct_permuted_better"] = round(pct_better, 1)
weak_seed_diagnostic["real_percentile_against_permuted"] = perm_dist_signed_sum_stats["real_percentile_against_permuted"]

# 4. Protective signal quality
sig_audit_ss = part3.get("B_signed_sum_online", {})
weak_seed_diagnostic["num_positive_weight_keys"] = sig_audit_ss.get("num_positive_weight_keys", 0)
weak_seed_diagnostic["num_negative_weight_keys"] = sig_audit_ss.get("num_negative_weight_keys", 0)
weak_seed_diagnostic["total_abs_positive_weight"] = round(sig_audit_ss.get("total_abs_positive_weight", 0.0), 4)
weak_seed_diagnostic["total_abs_negative_weight"] = round(sig_audit_ss.get("total_abs_negative_weight", 0.0), 4)
pos_weight = sig_audit_ss.get("total_abs_positive_weight", 0.0)
neg_weight = sig_audit_ss.get("total_abs_negative_weight", 0.0)
weight_ratio = pos_weight / neg_weight if neg_weight > 0 else float('inf')
weak_seed_diagnostic["positive_to_negative_weight_ratio"] = round(weight_ratio, 4)
weak_seed_diagnostic["protective_signal_helpful_count"] = sig_audit_ss.get("protective_signal_helpful_count", 0)
weak_seed_diagnostic["protective_signal_harmful_count"] = sig_audit_ss.get("protective_signal_harmful_count", 0)
weak_seed_diagnostic["protective_signal_net_helpful"] = protective_signal_net_helpful
candidates_with_signal = sig_audit_ss.get("candidates_with_protective_signal", 0)
candidates_changed = sig_audit_ss.get("candidates_where_negative_changed_selection", 0)
weak_seed_diagnostic["candidates_with_protective_signal"] = candidates_with_signal
weak_seed_diagnostic["candidates_where_negative_changed_selection"] = candidates_changed
if candidates_with_signal > 0:
    weak_seed_diagnostic["pct_candidates_where_negative_helped"] = round(
        100.0 * sig_audit_ss.get("protective_signal_helpful_count", 0) / candidates_with_signal, 1)
else:
    weak_seed_diagnostic["pct_candidates_where_negative_helped"] = 0.0

# 5. Online-vs-offline weight overlap analysis
online_risk_weights_nonzero = part5.get("online_risk_weights_nonzero", 0)
offline_risk_weights_nonzero = part5.get("offline_risk_weights_nonzero", 0)
overlap_keys = part5.get("overlap_top_10_online_offline_keys", 0)
weak_seed_diagnostic["online_risk_weights_nonzero"] = online_risk_weights_nonzero
weak_seed_diagnostic["offline_risk_weights_nonzero"] = offline_risk_weights_nonzero
weak_seed_diagnostic["online_vs_offline_nonzero_ratio"] = round(
    online_risk_weights_nonzero / offline_risk_weights_nonzero, 3) if offline_risk_weights_nonzero > 0 else 0.0
weak_seed_diagnostic["overlap_top_10_online_offline_keys"] = overlap_keys

# 6. Learning curve summary
learning_curve = part2.get("learning_curve", [])
episode_pvs = [ep["B_signed_sum_online_pv_episode"] for ep in learning_curve]
weak_seed_diagnostic["signed_sum_pv_per_episode"] = episode_pvs
weak_seed_diagnostic["signed_sum_pv_episode_range"] = max(episode_pvs) - min(episode_pvs) if episode_pvs else 0
weak_seed_diagnostic["signed_sum_pv_final_episode"] = episode_pvs[-1] if episode_pvs else signed_sum_pv
weak_seed_diagnostic["signed_sum_pv_best_episode"] = min(episode_pvs) if episode_pvs else signed_sum_pv
ep_a_pvs = [ep["A_total_pv_episode"] for ep in learning_curve]
weak_seed_diagnostic["a_pv_per_episode"] = ep_a_pvs

# 7. Aggregate assessment
# A weak seed is characterized by:
#   (a) offline upper bound substantially higher than other seeds (>=8)
#   (b) permuted distribution very tight (range <= 5)
#   (c) real PV ties with or is close to permuted median
high_offline_floor = offline_pv >= 8
tight_permuted_dist = permuted_range <= 5
real_near_permuted_median = signed_sum_pv <= perm_dist_signed_sum_stats["median_total_pv"] + 1
weak_seed_diagnostic["high_offline_floor_indicator"] = high_offline_floor
weak_seed_diagnostic["tight_permuted_distribution_indicator"] = tight_permuted_dist
weak_seed_diagnostic["real_near_permuted_median_indicator"] = real_near_permuted_median
weak_seed_indicators = sum([high_offline_floor, tight_permuted_dist, real_near_permuted_median])
weak_seed_diagnostic["weak_seed_indicator_count"] = weak_seed_indicators
weak_seed_diagnostic["weak_seed_confirmed"] = weak_seed_indicators >= 2
...
boolean_flag_details = {
    "shared_positions_used": {
        ...
        "supporting_values": {
            "position_seed_by_episode": list(episode_position_hashes.values()),
    ...
    "deterministic_permutation_seed_used": {
        "value": deterministic_permutation_seed_used,
    ...
    "no_oracle_leakage_confirmed": {
        ...
        "supporting_values": {
            "all_online_variants_clean": no_oracle_leakage_confirmed,
        },
    },
    ...
    "signed_sum_online_beats_permuted_median": {
        "value": signed_sum_online_beats_permuted_median,
        "rule": "B_signed_sum_online_total_pv < median(20 permuted repeats total_pv)",
        "supporting_values": {
            "signed_sum_pv": signed_sum_pv,
            "permuted_median": perm_dist_signed_sum_stats["median_total_pv"],
            "permuted_all": perm_dist_signed_sum_stats["all_permuted_pvs"],
        },
    },
    "signed_sum_online_beats_80pct_of_permuted": {
        "value": signed_sum_online_beats_80pct_of_permuted,
        "rule": "real PV better than >= 80% of permuted repeats",
        "supporting_values": {
            "num_worse": perm_dist_signed_sum_stats["num_permuted_worse_than_real"],
            "num_better": perm_dist_signed_sum_stats["num_permuted_better_than_real"],
        },
    },
    ...
    "signed_max_abs_online_beats_permuted_median": {
        "value": signed_max_abs_online_beats_permuted_median,
        "rule": "B_signed_max_abs_online_total_pv < median(20 permuted repeats total_pv)",
        "supporting_values": {
            "signed_max_pv": signed_max_pv,
            "permuted_median": perm_dist_signed_max_stats["median_total_pv"],
        },
    },
    "signed_max_abs_online_beats_80pct_of_permuted": {
        "value": signed_max_abs_online_beats_80pct_of_permuted,
        "rule": "real PV better than >= 80% of permuted repeats",
        "supporting_values": {
            "num_worse": perm_dist_signed_max_stats["num_permuted_worse_than_real"],
            "num_better": perm_dist_signed_max_stats["num_permuted_better_than_real"],
        },
    },
    ...
    "implementation_status": {
        ...
        "supporting_values": {
            ...
            "signed_sum_beats_permuted_median": signed_sum_online_beats_permuted_median,
        },
    },
    "failure_reason": {
...
output = {
    ...
    "weak_seed_diagnostic": weak_seed_diagnostic,
    "elapsed_seconds": round(elapsed, 1),
}

out_json_path = os.path.join(CURRENT_DIR, "runs",
    f"block1j26_seed103_weak_seed_diagnostic.json")
...
md_lines = []
md_lines.append("# Block 1J26 -- Seed 103 Weak-Seed Diagnostic for Online Signed Memory")
md_lines.append("")
...
md_lines.append("")
md_lines.append("**This is a deep-dive diagnostic on seed 103**, identified as a weak seed")
...
# Part 7: Weak-Seed Diagnostic (seed 103 specific)
md_lines.append("## 10. Part 7: Weak-Seed Diagnostic")
md_lines.append("")
md_lines.append("Deep-dive into why seed 103 shows weak signed-memory signal")
md_lines.append("compared to seeds 101 and 107.")
md_lines.append("")
md_lines.append("### 10.1 Baseline and Ceiling")
md_lines.append("")
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| A baseline total PV | {weak_seed_diagnostic['a_baseline_total_pv']} |")
md_lines.append(f"| Offline upper bound total PV | {weak_seed_diagnostic['offline_upper_bound_total_pv']} |")
md_lines.append(f"| Online-offline gap | {weak_seed_diagnostic['online_offline_gap']} |")
md_lines.append(f"| Offline norm PV | {weak_seed_diagnostic['offline_upper_bound_norm_pv']} |")
md_lines.append(f"| Offline dec PV | {weak_seed_diagnostic['offline_upper_bound_dec_pv']} |")
md_lines.append(f"| High offline floor (>={chr(61)}8) | {weak_seed_diagnostic['high_offline_floor_indicator']} |")
md_lines.append("")
md_lines.append("### 10.2 Signed Aggregation Divergence")
md_lines.append("")
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| signed_sum total PV | {weak_seed_diagnostic['signed_sum_total_pv']} |")
md_lines.append(f"| signed_max_abs total PV | {weak_seed_diagnostic['signed_max_abs_total_pv']} |")
md_lines.append(f"| PV divergence (max - sum) | {weak_seed_diagnostic['signed_sum_vs_signed_max_pv_divergence']} |")
md_lines.append(f"| signed_sum beats A | {weak_seed_diagnostic['signed_sum_beats_A']} |")
md_lines.append(f"| signed_max_abs beats A | {weak_seed_diagnostic['signed_max_abs_beats_A']} |")
md_lines.append("")
md_lines.append("### 10.3 Permuted Distribution (signed_sum)")
md_lines.append("")
md_lines.append(f"- n_permutation_repeats: {weak_seed_diagnostic['n_permutation_repeats']}")
md_lines.append(f"- min_pv: {weak_seed_diagnostic['permuted_min_pv']}, max_pv: {weak_seed_diagnostic['permuted_max_pv']}")
md_lines.append(f"- mean_pv: {weak_seed_diagnostic['permuted_mean_pv']}, median_pv: {weak_seed_diagnostic['permuted_median_pv']}")
md_lines.append(f"- std_pv: {weak_seed_diagnostic['permuted_std_pv']}, range: {weak_seed_diagnostic['permuted_distribution_range']}")
md_lines.append(f"- real_pv: {weak_seed_diagnostic['signed_sum_total_pv']}, real_percentile: {weak_seed_diagnostic['real_percentile_against_permuted']}")
md_lines.append(f"- num_worse: {weak_seed_diagnostic['num_permuted_worse_than_real']}, num_equal: {weak_seed_diagnostic['num_permuted_equal_to_real']}, num_better: {weak_seed_diagnostic['num_permuted_better_than_real']}")
md_lines.append(f"- pct_worse: {weak_seed_diagnostic['pct_permuted_worse']}%, pct_equal: {weak_seed_diagnostic['pct_permuted_equal']}%, pct_better: {weak_seed_diagnostic['pct_permuted_better']}%")
md_lines.append(f"- Tight permuted distribution: {weak_seed_diagnostic['tight_permuted_distribution_indicator']}")
md_lines.append(f"- Real near permuted median: {weak_seed_diagnostic['real_near_permuted_median_indicator']}")
md_lines.append("")
md_lines.append("### 10.4 Protective Signal Quality")
md_lines.append("")
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| num_positive_weight_keys | {weak_seed_diagnostic['num_positive_weight_keys']} |")
md_lines.append(f"| num_negative_weight_keys | {weak_seed_diagnostic['num_negative_weight_keys']} |")
md_lines.append(f"| total_abs_positive_weight | {weak_seed_diagnostic['total_abs_positive_weight']} |")
md_lines.append(f"| total_abs_negative_weight | {weak_seed_diagnostic['total_abs_negative_weight']} |")
md_lines.append(f"| pos/neg weight ratio | {weak_seed_diagnostic['positive_to_negative_weight_ratio']} |")
md_lines.append(f"| protective_signal_helpful | {weak_seed_diagnostic['protective_signal_helpful_count']} |")
md_lines.append(f"| protective_signal_harmful | {weak_seed_diagnostic['protective_signal_harmful_count']} |")
md_lines.append(f"| protective_signal_net_helpful | {weak_seed_diagnostic['protective_signal_net_helpful']} |")
md_lines.append(f"| candidates_with_signal | {weak_seed_diagnostic['candidates_with_protective_signal']} |")
md_lines.append(f"| candidates_changed_by_negative | {weak_seed_diagnostic['candidates_where_negative_changed_selection']} |")
md_lines.append(f"| pct where negative helped | {weak_seed_diagnostic['pct_candidates_where_negative_helped']}% |")
md_lines.append("")
md_lines.append("### 10.5 Online vs Offline Weight Growth")
md_lines.append("")
md_lines.append("| Metric | Value |")
md_lines.append("|--------|-------|")
md_lines.append(f"| online_risk_weights_nonzero | {weak_seed_diagnostic['online_risk_weights_nonzero']} |")
md_lines.append(f"| offline_risk_weights_nonzero | {weak_seed_diagnostic['offline_risk_weights_nonzero']} |")
md_lines.append(f"| online/offline nonzero ratio | {weak_seed_diagnostic['online_vs_offline_nonzero_ratio']} |")
md_lines.append(f"| overlap top-10 online/offline keys | {weak_seed_diagnostic['overlap_top_10_online_offline_keys']} |")
md_lines.append("")
md_lines.append("### 10.6 Learning Curve")
md_lines.append("")
md_lines.append(f"- signed_sum PV per episode: {weak_seed_diagnostic['signed_sum_pv_per_episode']}")
md_lines.append(f"- signed_sum PV episode range: {weak_seed_diagnostic['signed_sum_pv_episode_range']}")
md_lines.append(f"- signed_sum PV final episode: {weak_seed_diagnostic['signed_sum_pv_final_episode']}")
md_lines.append(f"- signed_sum PV best episode: {weak_seed_diagnostic['signed_sum_pv_best_episode']}")
md_lines.append(f"- A PV per episode: {weak_seed_diagnostic['a_pv_per_episode']}")
md_lines.append("")
md_lines.append("### 10.7 Aggregate Assessment")
md_lines.append("")
md_lines.append("| Indicator | Value |")
md_lines.append("|-----------|-------|")
md_lines.append(f"| High offline floor (>={chr(61)}8) | {weak_seed_diagnostic['high_offline_floor_indicator']} |")
md_lines.append(f"| Tight permuted distribution (range<={chr(61)}5) | {weak_seed_diagnostic['tight_permuted_distribution_indicator']} |")
md_lines.append(f"| Real near permuted median | {weak_seed_diagnostic['real_near_permuted_median_indicator']} |")
md_lines.append(f"| Weak seed indicator count (out of 3) | {weak_seed_diagnostic['weak_seed_indicator_count']} |")
md_lines.append(f"| **weak_seed_confirmed** | **{weak_seed_diagnostic['weak_seed_confirmed']}** |")
md_lines.append("")
md_lines.append("**Interpretation:** A confirmed weak seed has >=2 of these indicators:")
md_lines.append("1. High offline floor: the oracle itself cannot eliminate many PVs")
md_lines.append("2. Tight permuted distribution: real signal indistinguishable from noise")
md_lines.append("3. Real PV near permuted median: signed memory adds no value over random permutation")
md_lines.append("")
...
md_lines.append(f"block_id=1J26")
...
md_lines.append(f"signed_sum_permuted_median_pv={perm_dist_signed_sum_stats['median_total_pv']}")
md_lines.append(f"signed_sum_permuted_min_pv={perm_dist_signed_sum_stats['min_total_pv']}")
md_lines.append(f"signed_sum_permuted_max_pv={perm_dist_signed_sum_stats['max_total_pv']}")
md_lines.append(f"signed_sum_real_percentile_against_permuted={perm_dist_signed_sum_stats['real_percentile_against_permuted']}")
md_lines.append(f"signed_max_abs_permuted_median_pv={perm_dist_signed_max_stats['median_total_pv']}")
md_lines.append(f"signed_max_abs_permuted_min_pv={perm_dist_signed_max_stats['min_total_pv']}")
md_lines.append(f"signed_max_abs_permuted_max_pv={perm_dist_signed_max_stats['max_total_pv']}")
md_lines.append(f"signed_max_abs_real_percentile_against_permuted={perm_dist_signed_max_stats['real_percentile_against_permuted']}")
...
md_lines.append(f"signed_sum_online_beats_permuted_median={signed_sum_online_beats_permuted_median}")
md_lines.append(f"signed_max_abs_online_beats_A={signed_max_abs_online_beats_A}")
md_lines.append(f"signed_max_abs_online_beats_permuted_median={signed_max_abs_online_beats_permuted_median}")
...
md_lines.append(f"n_permutation_repeats={N_PERM_REPEATS}")
md_lines.append(f"offline_upper_bound_pv={offline_pv}")
md_lines.append(f"online_offline_gap={online_gap}")
md_lines.append(f"signed_sum_vs_signed_max_pv_divergence={signed_sum_vs_signed_max_pv_divergence}")
md_lines.append(f"permuted_distribution_range={permuted_range}")
md_lines.append(f"pct_permuted_worse={weak_seed_diagnostic['pct_permuted_worse']}")
md_lines.append(f"pct_permuted_equal={weak_seed_diagnostic['pct_permuted_equal']}")
md_lines.append(f"positive_to_negative_weight_ratio={weak_seed_diagnostic['positive_to_negative_weight_ratio']}")
md_lines.append(f"protective_signal_net_helpful={protective_signal_net_helpful}")
md_lines.append(f"high_offline_floor_indicator={high_offline_floor}")
md_lines.append(f"tight_permuted_distribution_indicator={tight_permuted_dist}")
md_lines.append(f"real_near_permuted_median_indicator={real_near_permuted_median}")
md_lines.append(f"weak_seed_indicator_count={weak_seed_indicators}")
md_lines.append(f"weak_seed_confirmed={weak_seed_diagnostic['weak_seed_confirmed']}")
md_lines.append(f"elapsed={elapsed:.1f}s")
md_lines.append("```")

out_md_path = os.path.join(CURRENT_DIR, "protocols",
    f"block1j26_seed103_weak_seed_diagnostic.md")

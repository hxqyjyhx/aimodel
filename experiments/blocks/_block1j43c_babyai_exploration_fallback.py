"""
Block 1J43c -- BabyAI Exploration Fallback.

This is a BabyAI probe repair focused on target-not-visible behavior. It keeps
the legal observation / action / outcome conversion and visible-target bridge
from 1J43b, but replaces the random fallback with a legal local exploration
fallback based on partial observations, recent observation signatures, and
...
SCRIPT_NAME = "_block1j43c_babyai_exploration_fallback.py"
RUN_JSON = os.path.join(CURRENT_DIR, "runs", "block1j43c_babyai_exploration_fallback.json")
REPORT_MD = os.path.join(CURRENT_DIR, "protocols", "block1j43c_babyai_exploration_fallback.md")
TABLE_CSV = os.path.join(CURRENT_DIR, "protocols", "block1j43c_babyai_exploration_fallback_table.csv")
CHECKPOINT_MD = os.path.join(CURRENT_DIR, "checkpoint_1j43c_babyai_exploration_fallback.md")
...
def run_env_seed(env_id, seed):
    ...
    return {
        ...
        "target_visible_rate": policy_summary["exploration_fallback_probe"]["target_visible_rate"],
        "target_adjacent_rate": policy_summary["exploration_fallback_probe"]["target_adjacent_rate"],
        "action_bridge_coverage": policy_summary["exploration_fallback_probe"]["action_bridge_coverage"],
        ...
        "exploration_fallback_probe_return": policy_summary["exploration_fallback_probe"]["return"],
        "delta_vs_random": policy_summary["exploration_fallback_probe"]["return"] - policy_summary["random_policy"]["return"],
        "delta_vs_1j43b": policy_summary["exploration_fallback_probe"]["return"] - policy_summary["1j43b_experience_plus_mission_probe"]["return"],
        "success_proxy_rate": policy_summary["exploration_fallback_probe"]["success_proxy_rate"],
        "visible_target_success_rate": policy_summary["exploration_fallback_probe"]["visible_target_success_rate"],
        "target_not_visible_fallback_success_rate": policy_summary["exploration_fallback_probe"][
            "target_not_visible_fallback_success_rate"
        ],
        "target_discovery_rate": policy_summary["exploration_fallback_probe"]["target_discovery_rate"],
        "steps_until_target_visible": policy_summary["exploration_fallback_probe"]["steps_until_target_visible"],
        "repeated_observation_rate": policy_summary["exploration_fallback_probe"]["repeated_observation_rate"],
        "stuck_loop_count": policy_summary["exploration_fallback_probe"]["stuck_loop_count"],
        "wall_collision_or_blocked_forward_count": policy_summary["exploration_fallback_probe"][
            "wall_collision_or_blocked_forward_count"
        ],
        "new_observation_signature_rate": policy_summary["exploration_fallback_probe"]["new_observation_signature_rate"],
        "timeout_rate": policy_summary["exploration_fallback_probe"]["timeout_rate"],
        "action_distribution": policy_summary["exploration_fallback_probe"]["action_distribution"],
...
exploration_fallback_return = agg("exploration_fallback_probe_return")
...
any_nonzero_env = any(
    row["exploration_fallback_probe_return"] > 0.0
    or row["success_proxy_rate"] > 0.0
    or row["target_discovery_rate"] > 0.0
    for row in all_rows
) if all_rows else False
improves_over_1j43b = exploration_fallback_return > probe_1j43b_return or target_discovery_rate > 0.0
...
if (
    ...
    and improves_over_1j43b
    and (target_not_visible_fallback_success_rate > 0.0 or target_discovery_rate > 0.0)
    and any_nonzero_env
):
    status = "PASS"
elif all_rows and not hidden_leakage and (improves_over_1j43b or repeated_observation_rate < 1.0):
    status = "PARTIAL"
else:
    status = "FAIL"
...
payload = {
    ...
    "summary": {
        ...
        "exploration_fallback_probe_return": exploration_fallback_return,
        ...
        "main_exploration_failure": "target-not-visible episodes still rely on local novelty only and cannot route around obstacles globally",
...
with open(TABLE_CSV, "w", encoding="utf-8", newline="") as handle:
    writer = csv.DictWriter(
        ...
        fieldnames=[
            ...
            "random_return",
            "1j43b_probe_return",
            "exploration_fallback_probe_return",
            "delta_vs_random",
            "delta_vs_1j43b",
            "success_proxy_rate",
...
report_lines = [
    ...
    f"- Exploration fallback return: `{exploration_fallback_return:.4f}`",
    f"- Delta vs random: `{delta_vs_random:+.4f}`",
    f"- Delta vs 1J43b: `{delta_vs_1j43b:+.4f}`",
    ...
    f"- Steps until target visible: `{steps_until_target_visible:.4f}`",
    "",
    "## Exploration",
    ...
    f"- Main exploration failure: `target-not-visible episodes still rely on local novelty only and cannot route around obstacles globally`",
    "",
    "## Mechanism",
...
checkpoint_lines = [
    ...
    f"- Mission parse success: `{mission_parse_success_rate:.4f}`",
    f"- Hidden leakage: `{str(hidden_leakage).lower()}`",
    "- Next step: BabyAI small pilot if target-not-visible behavior now improves; otherwise refine local exploration fallback.",
]
...
print(
    "Policy Probe: "
    f"random={random_return:.4f} probe_1j43b={probe_1j43b_return:.4f} explore={exploration_fallback_return:.4f} "
    f"delta_random={delta_vs_random:+.4f} delta_1j43b={delta_vs_1j43b:+.4f} success={success_proxy_rate:.4f} "
